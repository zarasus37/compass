...
/**
 * Edit goal form (Cluster 1.10).
 *
 * Same shape as the new goal form, but pre-filled with the
 * existing goal's values. The hidden goalId field routes the
 * saveGoalEdit action to the right record.
 */
...
import { saveGoalEdit, type AddGoalResult } from "@/app/actions/goals";
    ...
    <form action={formAction} style={{ display: "grid", gap: 24 }}>
      ...
      >
        <div style={{ display: "grid", gap: 22 }}>
          <Field
            label="Name"
            hint="The destination — keep it the same or change it."
          >
            <input
              type="text"
              name="name"
          ...
          <Field label="Per paycheck" hint="How much to set aside every payday.">
            <MoneyInput
              name="perPaycheck"
              defaultValue={(goal.perPaycheckCents / 100).toFixed(2)}
              required
            />
          </Field>
          ...
          <Field
            label="Linked vessel"
            hint="The envelope that will hold this goal's money."
          >
            <select
              name="envelopeId"
            ...
            {PLANET_OPTIONS.map((p) => (
              ...
              >
                <input
                  ...
                  name="planet"
                  value={p.id}
                  defaultChecked={p.id === goal.planet}
                  style={{
