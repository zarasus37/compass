...
**Date**: 2026-08-30 22:25 CDT
**Last commit**: Cluster 7.8.1 (Audit log retention cron) — committed on top of `8f7b23b` (Cluster 7.8)
**Predecessor commit**: `8f7b23b` (Cluster 7.8) → `ef0982a` (Cluster 7.7) → `3af7566` (Cluster 7.6) → `f980240` (session-scratch cleanup) → `eb7c1f9` (Cluster 7.5) → `ed4129b` (handoff + COORDINATION reflect Cluster 7.4) → `ee405f8` (Cluster 7.4)
...
Compass is at a clean natural breakpoint. The most recent work was **Cluster 7.8.1 — Audit log retention cron (M8)**: a nightly job that calls `pruneAuditLog` for every user, with the same pattern as the existing vault cron. New `pruneAuditLogForAllUsers(opts?)` bulk helper (per-user try/catch so one user's failure doesn't abort the batch); new `POST /api/cron/audit-log-prune` endpoint (gated by the existing `CRON_SECRET` bearer — skipped in dev); new `scripts/cron-audit-prune-dev.mjs` poller (configurable via `AUDIT_LOG_PRUNE_POLL_MS` env, default 24h); new `cron:dev:audit` npm script. The middleware gained `/api/cron` in the public prefixes list — a pre-existing bug where `scripts/cron-dev.mjs` (the vault scheduler) was being silently redirected to `/login` is now fixed too. New `tests/smoke-cron-audit-log-prune.mjs` (30 checks) covers happy path, idempotency, per-user counts, DB state, source-file checks, and POST+GET wire checks. New `tests/integration-vault.mjs` Phase 4.0 M8 (23 checks) verifies the bulk helper, route, dev scheduler script, middleware fix, npm script, and POST+GET wire. `.env.production.example` documents the optional `CRON_SECRET` env var (generate with `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`). No visible-UI payoff (pure infra); the user sees nothing new, but the live `AuditLog` table is now bounded at ~90 days per user. All 19 data-layer smokes + 13 UI smokes + integration-vault 226 + smoke-deploy = ~1,620 checks, ALL GREEN (the 3 pre-existing audit-log misses in 7.7 — date-TZ edge cases — are unchanged). tsc clean. Dev server live on testnet default. The production schedule (Vercel cron `0 3 * * *` recommended) is documented in the spec but not wired — adding `vercel.json` is a future cluster.
