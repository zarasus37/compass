...
**Date**: 2026-08-31 05:25 CDT
**Last commit**: Cluster 7.11.1 (Live activity ticker polish — semantic tones + reconcile-on-reconnect) — committed on top of `a8639d6` (Cluster 7.11)
...
Compass is at a clean natural breakpoint. The most recent work was **Cluster 7.11.1 — Live activity ticker polish (semantic tones + reconcile-on-reconnect)**: three real wins from Polar's v2-amended spec, folded in as a polish commit on top of the 7.11 ticker that shipped earlier today. (1) **Semantic tone colors** — the ticker's color dot now signals "what kind of news is this" (good=green `--ok`, watch=orange `--vessel-watch`, bad=red `--vessel-over`, neutral=dim `--ink-3`) via a new `HumanizeTone` type + `TONE_FOR` `Record<VaultAuditActionType, HumanizeTone>` map + `TONE_COLOR` CSS-var map in `audit-log-shared.ts`. The djb2 hash (`colorForActionType`) stays in use by the audit page where 50+ row type-distinct rendering earns its keep; in a 3-row ticker it was meaningless confetti. (2) **Hide unmapped types** — `humanizeVaultAction` return shape changed from `string` to `{ text, tone } | null`; the ticker silently drops `null` (the future-type safety net) instead of rendering an "event happened" placeholder. (3) **Reconcile on reconnect** — `useAuditStream`'s docblock is explicit that reconnect does NOT replay missed events; the ticker now hits the new `GET /api/vault/audit/recent?take=N` endpoint on the `reconnecting`/`closed` → `live` transition to backfill any rows that arrived during the disconnect window. New `LiveTickerEvent.tone` field carries the semantic tone to the render; the ticker renders `data-tone="good|watch|bad|neutral"` on every row for the smoke to assert. Updated `tests/smoke-live-ticker.mjs` (64 checks) covers tone rendering + the new endpoint round-trip + reconcile source-file checks. `tests/integration-vault.mjs` Phase 4.0 M11 grew from 35 to 53 checks (added HumanizeTone, TONE_COLOR, TONE_FOR exhaustiveness, new endpoint, reconcile wiring). All 20 data-layer smokes (~1,070 checks) + integration-vault 310 + smoke-deploy 102 = **~1,800 checks, ALL GREEN** (the pre-existing `smoke-goals-db.mjs` seed-order flake is unrelated to 7.11.1; reproduces on the un-modified `a8639d6` branch). tsc clean. Visible-UI payoff: the ticker dots now read as "what kind of event" instead of "which type" — the user can triage at a glance (red=needs me, green=good news, orange=in progress, dim=ambient), and a momentary SSE disconnect no longer drops events. (Previous 7.11 shipped the ticker itself; see that section for the wiring.) Predecessor commit: `a8639d6` (Cluster 7.11). Spec: no new spec file; this commit is the polish.
