...
**Date**: 2026-08-29 07:00 CDT
**Last commit**: `29bc14f` — *Cluster 7.2 — Recent items in ⌘K palette (visible UI) (2026-08-29).*
**Predecessor commit**: `744d32d` (Cluster 7.1 docs) → `d71c224` (Cluster 7.1 — ⌘K command palette) → `f3332d7` (Cluster 7.0 docs) → `38eb7e8` (Cluster 7.0 — Vault preferences hub) → `a23031c` (Cluster 6.0 fix-up docs) → `8ef4f8f` (Cluster 6.0 fix-up — VaultSchedule in Prisma schema) → `8c97dbf` (Cluster 6.0.1 docs) → `37dc04b` (Cluster 7.0 — Vault preferences hub) → `6393645` (spec) → `2f06d0b` (Cluster 6.0 docs) → `62b8528` (Cluster 6.0 — Vault scheduler) → `e23503c` (smoke:all wiring)

---

## TL;DR

Compass is at a clean natural breakpoint. The most recent work was **Cluster 7.2 — Recent items in ⌘K palette** (commit `29bc14f`): a small follow-on to Cluster 7.1. The ⌘K palette now shows a `// RECENT · N items` section above the result list when the query is empty and the user has navigated via the palette before. The RECENT list is local (localStorage, key `compass-palette-recent`), capped at 8 items, dedups by id, persists across sessions. New `recent-items.ts` module with three exports (getRecent, pushRecent, clearRecent), SSR-safe. The `CommandPalette` now reads the recent list on open via useEffect, renders the section header, and calls pushRecent(item) before closing. The /api/command-palette endpoint adds `recentCount: 0` to the wire format. **All 28 smokes still green via `pnpm smoke:all`** (~1,432 checks; the `smoke-command-palette` is now 77 checks, was 64), `tsc` clean, dev server live on testnet default.

The previous cluster was **7.1 — ⌘K command palette** (commit `d71c224`): a global search/jump drawer. ⌘K (or Ctrl+K) opens a centered palette anywhere on a signed-in page; type to filter across 21 static routes + the ~30 envelopes/goals/debts/bills/accounts in the DB; arrow keys navigate, Enter jumps, Escape closes. Pure client-side filtering on a server-provided index (no round-trip per keystroke). New `CommandPalette` modal + `CommandPaletteProvider` (open/close + global keyboard) + `SearchButton` (the discoverability affordance in the top bar) + `match.ts` (pure case-insensitive substring + word-boundary-boost matcher) + `getSearchIndex` server function. New `GET /api/command-palette?q=…` endpoint for the smoke + future network-shared palette. The dashboard gets its own provider since it lives outside `(app)/`.
