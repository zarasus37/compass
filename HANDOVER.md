...
**Date**: 2026-08-29 05:45 CDT
**Last commit**: `d71c224` — *Cluster 7.1 — ⌘K command palette (visible UI) (2026-08-29).*
**Predecessor commit**: `f3332d7` (Cluster 7.0 docs) → `38eb7e8` (Cluster 7.0 — Vault preferences hub) → `a23031c` (Cluster 6.0 fix-up docs) → `8ef4f8f` (Cluster 6.0 fix-up — VaultSchedule in Prisma schema) → `8c97dbf` (Cluster 6.0.1 docs) → `37dc04b` (Cluster 6.0.1 — Vault mainnet) → `6393645` (spec) → `2f06d0b` (Cluster 6.0 docs) → `62b8528` (Cluster 6.0 — Vault scheduler) → `e23503c` (smoke:all wiring)

---

## TL;DR

Compass is at a clean natural breakpoint. The most recent work was **Cluster 7.1 — ⌘K command palette** (commit `d71c224`): a global search/jump drawer. ⌘K (or Ctrl+K) opens a centered palette anywhere on a signed-in page; type to filter across 21 static routes + the ~30 envelopes/goals/debts/bills/accounts in the DB; arrow keys navigate, Enter jumps, Escape closes. Pure client-side filtering on a server-provided index (no round-trip per keystroke). New `CommandPalette` modal + `CommandPaletteProvider` (open/close + global keyboard) + `SearchButton` (the discoverability affordance in the top bar) + `match.ts` (pure case-insensitive substring + word-boundary-boost matcher) + `getSearchIndex` server function. New `GET /api/command-palette?q=…` endpoint for the smoke + future network-shared palette. The dashboard gets its own provider since it lives outside `(app)/`. **All 28 smokes green via `pnpm smoke:all`** (13 data-layer + 13 UI + 1 integration-vault + 1 deploy = 28; ~1,419 checks; the new `smoke-command-palette` is the 13th data-layer), `tsc` clean, dev server live on testnet default.

The previous cluster was **7.0 — Vault preferences hub** (commit `38eb7e8`): new `/vault/preferences` page consolidates the user's vault policy in one place. 4-cell policy summary (yield routing, risk disclosure, auto bill-pay, vault status) + yield routing picker + risk disclosure section (with new re-acknowledge flow) + schedule summary card. New `RevokeRiskAckButton` + `revokeRiskDisclosureAction` server action round-trip the suppression. Re-skinned `RiskDisclosure` unack state + `SchedulerIndicator` + schedule page chrome to vessel tokens. Sidebar adds the `Preferences` entry under `// Ledger`. The `Vault` item now has `match: "exact"` so it doesn't light up on `/vault/preferences` or `/vault/schedule`.
