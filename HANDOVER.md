...
# Local agent prompt — pull cluster 7.17 from sandbox into local clone and push

Copy-paste the block below into your local agent (Claude Code, Cursor, etc.) running against the Compass repo on the Windows machine.

---

```
Context: A sandbox session made a "Cluster 7.17: production-readiness hardening" commit (756b832) against the Compass repo on a Linux sandbox. The commit is sitting on `main` locally in the sandbox but the sandbox has no GitHub credentials, so it wasn't pushed. Your job is to land that commit on the real `origin/main` for `zarasus37/compass`.

The Linux sandbox is at /workspace/compass. You can't reach it directly from Windows. Use the bridge below to copy the changes into your local clone.

Files changed (14 total):

  Modified:
    00-MOM-LAUNCH-RUNBOOK.md
    HANDOVER.md
    package.json
    prisma/migrations/migration_lock.toml
    scripts/seed-admin.mjs
    src/app/(app)/obligations/page.tsx
    src/app/(app)/period/page.tsx
    src/generated/prisma/runtime/client.d.ts         (auto-generated; skip)
    src/generated/prisma/runtime/index-browser.d.ts  (auto-generated; skip)
    tests/integration-vault.mjs

  Deleted:
    prisma/migrations/20260822044950_scaffold_initial_user/migration.sql
    prisma/migrations/20260822051506_add_session_model/migration.sql
    prisma/migrations/20260822113455_cluster1_full_data_model/migration.sql

  Created:
    prisma/migrations/20260922072521_init/migration.sql

The patch is ~4,700 insertions. The two generated files (src/generated/prisma/runtime/*) are auto-regenerated by `pnpm prisma generate`; you can skip copying them and regenerate locally instead.

Step 1 — Get the patch into your local clone

Option A (preferred): ask the sandbox operator to drop the patch file at a known path, then:
...
Option B (if patch provided): apply it:

  git apply --check cluster-7.17.patch   # dry run
  git apply cluster-7.17.patch
  git status   # should show the 14 files above (minus the 2 generated ones if you skip them)

Option C (manual copy): for each file in the "Modified" + "Created" + "Deleted" lists above, copy from the sandbox's /workspace/compass/ to your local clone. Don't copy the 2 generated files.

Step 2 — Regenerate the Prisma client locally

The two src/generated/prisma/runtime/* files are auto-generated; don't try to copy them byte-for-byte (they may differ between Prisma versions). Just regenerate:
...
Step 3 — Sanity-check before committing
  ...
  pnpm tsc
  pnpm lint    # expect lots of warnings, ignore them; only fail on errors
  pnpm build   # should complete with "Compiled successfully" + 47 routes

If tsc fails, report back with the error — don't force-merge a broken state.

Step 4 — Commit on a feature branch

Do NOT commit straight to main. Use a feature branch so the diff is reviewable:

  git checkout -b cluster-7.17-production-readiness
  git add -A
  git status   # verify the 14 files are staged (minus generated) and the 3 deletions are staged
  git commit -m "Cluster 7.17: production-readiness hardening (migrations + bug fixes + runbook)

Replace stale SQLite-syntax migrations with a single Postgres-syntax init
migration (20260922072521_init) so \`prisma migrate deploy\` works on a
fresh Neon DB. The previous migrations were written before the
Postgres-everywhere switch (cluster 2026-08-28) and used DATETIME /
CURRENT_TIMESTAMP syntax that Postgres would reject. migration_lock.toml
updated to provider = postgresql.

Add \`db:migrate\` and \`db:migrate:deploy\` scripts; \`pnpm build\` now
runs \`prisma generate && next build\` explicitly.

Fix React 19 \`<title>\` array-children warnings in obligations/page.tsx
and period/page.tsx (SVG tooltip tags had multi-expression JSX children).

Update integration-vault M9 assertion to accept both Vercel Pro
(*/N<=5) and Hobby (X H * * *) vault cron schedules — fixes the stale
smoke that was failing after commit ea1d49a intentionally changed the
vault cron to Hobby-daily.

Update 00-MOM-LAUNCH-RUNBOOK.md with Step 6.5: after first deploy, the
operator clicks /settings → Reset to seed data to populate mom's
dashboard with the 7 vessels, 6 bills, 4 goals, allocation plan, and
financial identity. seed-admin only creates the user (server-only
helpers can't run in a Node CLI).

Verified end-to-end on a fresh Postgres:
- prisma migrate deploy applies 20260922072521_init
- seed-admin creates the user row
- _prisma_migrations is populated
- /api/health reports migrationStatus: current"

Step 5 — Push and open the PR

  git push -u origin cluster-7.17-production-readiness
  gh pr create --base main --title "Cluster 7.17: production-readiness hardening" --body "..."

PR body should reference:
- Sandbox commit 756b832
- HANDOVER.md session entry (lines 460-778)
- The 3 critical fixes: migration rewrite, Hobby/Pro cadence, React <title> warnings

CI will run pnpm tsc + the smoke suite. If any smoke fails, that's a regression — don't merge. If it's the known flaky smoke-live-ticker off-by-one (documented in the handover), that's expected.

Step 6 — Merge

After CI is green and you've eyeballed the diff, merge to main. Vercel's preview deploy will pick up the change automatically. Then proceed with 00-MOM-LAUNCH-RUNBOOK.md Steps 1-7 to ship to mom.

If anything in this prompt is unclear, ask before doing irreversible damage to the repo. The sandbox commit is the source of truth — `git show 756b832` on origin (after you push) shows the exact diff.
```

---

End of local-agent prompt. The full cluster-7.17 audit context is in HANDOVER.md above this section.
