...
**Date**: 2026-08-30 20:30 CDT
**Last commit**: Cluster 7.8 (Audit log retention) — committed on top of `ef0982a` (Cluster 7.7)
**Predecessor commit**: `ef0982a` (Cluster 7.7) → `3af7566` (Cluster 7.6) → `f980240` (session-scratch cleanup) → `eb7c1f9` (Cluster 7.5) → `ed4129b` (handoff + COORDINATION reflect Cluster 7.4) → `ee405f8` (Cluster 7.4)
...
Compass is at a clean natural breakpoint. The most recent work was **Cluster 7.8 — Audit log retention**: the live `AuditLog` table now holds only the last 90 days of events; older events are aggregated into a new `AuditLogDailyRollup` table (one row per `(userId, dateKey, actionType)` with `count` and `failedCount`) by a `pruneAuditLog(userId, options)` function. The prune is idempotent (upsert with `increment`); a nightly cron is the next cluster. New dev-only `POST /api/dev/audit-log-prune` endpoint lets the smoke exercise the rollup in seconds (`retentionDays: 0` rolls up everything). The `getAuditLogActivity` data-layer function was refactored to read from BOTH the live table AND the rollup — the 30-day strip is unchanged (within the live horizon); the new "Last 12 months" chip drives a 365-day strip whose first 90 days come from live rows and the rest from the rollup. The activity strip's geometry is now dynamic (`geometryFor` + `downsample` helpers): ≤30 days keeps the original 30-bar layout, 31-90 days widens the SVG to fit 90 daily bars at 7px each, >90 days downsamples to 90 buckets (each bucket spans ~N/90 days). The strip's header label follows the active window (`// 30-day shape` → `// 90-day shape` → `// 365-day shape`) and a new `data-window-days` testid is set on the strip div. The DateRangeBar gained a 6th chip ("Last 12 months", `daysBack: 365`); the page's `computeActivityDays` helper derives the strip's window from the active `from`/`to` filter. New `tests/smoke-audit-log-retention.mjs` (44 checks) covers prune correctness, idempotency, re-insert increment, FAILED-set rollup, dev endpoint gating, 90/365-day view rendering, and ActivityStrip dynamic geometry. New `tests/integration-vault.mjs` Phase 4.0 M7 (20 checks) verifies the Prisma model, table, unique index, dev endpoint, `pruneAuditLog` + `getRetentionDays` exports, 365d preset, and dynamic strip. All 18 data-layer smokes + 13 UI smokes + integration-vault 203 + smoke-deploy = ~1,590 checks, ALL GREEN (the 3 pre-existing audit-log misses in 7.7 — date-TZ edge cases unrelated to 7.8 — are unchanged). tsc clean. Dev server live on testnet default.
...
- **Pre-existing 7.7 misses in `smoke-audit-log.mjs` are NOT from this cluster.** Three checks fail (3 / 65): the `?from=<rowDate> includes the sentinel row` check, the `?from + ?type composes (AND)` check, and the `range dimming — at least 20 bars are out-of-range (narrow 5d window)` check. All three are date-TZ edge cases: the most recent sentinel's UTC date can be one day ahead of the `?from=` local-date, so the test's UTC-vs-local comparison fails. The same 3 misses existed in the pre-7.8 baseline (verified by stashing the cluster's changes and re-running). The C7.8 spec lists this as a pre-existing issue to fix in a follow-on cluster (cluster 7.9 candidate).
...
| Smoke scripts | `tests/smoke-*.mjs` (32 files, including `smoke-audit-log-retention.mjs`) + `tests/integration-vault.mjs` (now includes M7 phase) + `tests/smoke-deploy.mjs` |
