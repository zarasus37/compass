...
**Date**: 2026-08-30 07:35 CDT
**Last commit**: Cluster 7.7 (Date range filter on `/vault/audit`) — committed on top of `3af7566` (Cluster 7.6) → `f980240` (session-scratch cleanup) → `eb7c1f9` (Cluster 7.5) → `ed4129b` (handoff + COORDINATION reflect Cluster 7.4)
**Predecessor commit**: `3af7566` (Cluster 7.6) → `f980240` (session-scratch cleanup) → `eb7c1f9` (Cluster 7.5) → `ed4129b` (handoff + COORDINATION reflect Cluster 7.4) → `ee405f8` (Cluster 7.4)
...
Compass is at a clean natural breakpoint. The most recent work was **Cluster 7.7 — Date range filter on `/vault/audit`**: the audit-log page now has a row of 5 preset chips (Last 24h / 7d / 30d / 90d / All time) that scope the entire page to a date window via `?from=YYYY-MM-DD&to=YYYY-MM-DD`. New `DateRangeBar` component (server-rendered, chips are plain `<Link>`s that merge the new range with the rest of the page's filter). `AuditLogFilter` extended with `from` / `to` strings; `parseAuditLogFilter` validates YYYY-MM-DD dates (silently drops malformed values, drops `to` when `from > to`); `whereFromFilter` in the server-only module adds `createdAt: { gte: from, lt: to + 1 day }` at the DB layer; `rowMatchesAuditFilter` (the predicate the live wrapper uses) was extended to gate on the date range — so the C7.6 SSE live-update wiring filters streamed rows through the same predicate with no new code. The 30-day activity strip dims out-of-range bars to 30% opacity (each bar gets a `data-in-range` testid); the section header flips from "Last 30 days" to "Last 30 days (range dimmed)" when a range is active. The 4-cell headline strip stays UNFILTERED (per the 7.4 contract). New preset constant `DATE_RANGE_PRESETS` + helpers `dateRangeForPreset`, `parseYmdDate`, `toYmd`, `hasDateRange`. New `tests/smoke-audit-log.mjs` checks (24 new — 64 total, up from 36) cover chip rendering, href correctness, range + type composition, range dimming, CLEAR chip, malformed dates, `from > to` forgiveness. All 17 data-layer smokes + 13 UI smokes + integration + deploy = **31 suites, ~1,546 checks, ALL GREEN**. tsc clean. Dev server live on testnet default.
...
| **Date range filter (C7.7)** | `src/app/(app)/vault/audit/DateRangeBar.tsx` (preset chips + active-range badge) + `src/lib/vault/audit-log-shared.ts` (extends `AuditLogFilter` with `from` / `to`; adds `DATE_RANGE_PRESETS`, `dateRangeForPreset`, `parseYmdDate`, `toYmd`, `hasDateRange`) + `src/lib/vault/audit-log.ts` (`whereFromFilter` adds `createdAt: { gte, lt }` when set) + `src/app/(app)/vault/audit/ActivityStrip.tsx` (dims out-of-range bars to 30% opacity via new `rangeFrom` / `rangeTo` props) |
| **Date range filter (C7.7)** | `src/app/(app)/vault/audit/DateRangeBar.tsx` (preset chips + active-range badge) + `src/lib/vault/audit-log-shared.ts` (extends `AuditLogFilter` with `from` / `to`; adds `DATE_RANGE_PRESETS`, `dateRangeForPreset`, `parseYmdDate`, `toYmd`, `hasDateRange`) + `src/lib/vault/audit-log.ts` (`whereFromFilter` adds `createdAt: { gte, lt }` when set) + `src/app/(app)/vault/audit/ActivityStrip.tsx` (dims out-of-range bars to 30% opacity via new `rangeFrom` / `rangeTo` props) |
