...
**Date**: 2026-08-31 00:50 CDT
**Last commit**: Cluster 7.9 (Audit log smoke fix) — committed on top of `52bb94c` (Cluster 7.8.2)
**Predecessor commit**: `52bb94c` (Cluster 7.8.2) → `b7ef8cf` (Cluster 7.8.1) → `8f7b23b` (Cluster 7.8) → `ef0982a` (Cluster 7.7) → `3af7566` (Cluster 7.6) → `f980240` (session-scratch cleanup) → `eb7c1f9` (Cluster 7.5) → `ed4129b` (handoff + COORDINATION reflect Cluster 7.4) → `ee405f8` (Cluster 7.4)
...
Compass is at a clean natural breakpoint. The most recent work was **Cluster 7.9 — Audit log smoke fix**: the C7.7 dimming test (`?from=<5d-ago>&to=<today>` → expect ≥20 out-of-range bars) was obsolete after C7.8's "strip scales with the range" behavior change. A `from=+to=` filter makes the strip exactly the window (all in-range, none out-of-range), so the test's premise no longer held. Fix: change the dimming test to use a from-only request (`?from=<5d-ago>`, no `to`) which keeps the strip at its 30-day default while the filter is open-ended — the 25 bars before `5d-ago` are out-of-range, the 5 bars from `5d-ago` to today are in-range. No visible-UI payoff (pure smoke fix); the user sees nothing different. All 19 data-layer smokes (915 checks) + integration-vault 239 + smoke-deploy 102 = **~1,680 checks, ALL GREEN — the baseline is back to 100%**. tsc clean. Dev server live on testnet default. The date-TZ edge cases in tests (1) and (2) (`?from=<rowDate>` + `?from+?type composes`) pass naturally when local and UTC dates align; they're a latent flakiness issue, not a C7.8 regression — a follow-on cluster could pin `now` to noon local if the flakiness becomes a real problem.
