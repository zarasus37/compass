...
> Last update: 2026-08-25 (post-Cluster-5.0-Part-B — Prisma persistence: FinancialIdentity + 7 child tables + OnboardingMessage log; L1 rules fallback in `src/lib/llm/index.ts` fires when Mavis/Ollama error and the deterministic mock engine produces the response; `OnboardingState` interface unchanged but `loadConversation`/`saveConversation` are now async; new `lastFellBack` + `lastErrorMessage` fields; new `GET /api/dev-agent/run-agent?userId=...` endpoint for smoke to verify persistence; new `/api/dev-agent/test-llm-call` endpoint for smoke to force a Mavis/Ollama failure and assert the L1 fallback fires; smoke expanded from 42 → 80 checks; total 15 smokes at 551/551)
...
- **Cluster 5.1 — Chat UX**: `OnboardingGate` in (app) layout, `/onboarding` page, `ChatSurface` component, `ProgressRail` for milestone progress. Privacy boundary copy ("your financial data stays on this device after this conversation ends"). The persistence is ready — the UI just needs to call `runAgent` and render the messages.
- **Cluster 5.2 — Wire dashboard to identity**: every surface reads from the materialized records (income, debts, goals, etc.). Seed moves to "demo mode" — a banner on /onboarding lets the user load the seed instead of going through the conversation.
...
- **From session**: `mvs_1bcddf951dde44e08752858a24aeda18` (Cluster 5.0 Part B — Prisma persistence: 9 new tables, LLM dispatcher L1 rules fallback, `OnboardingState.lastFellBack` + `lastErrorMessage`, GET endpoint for smoke to verify persistence, /test-llm-call endpoint for smoke to force the fallback, onboarding smoke 42 → 80 checks; total 15 smokes at 551/551)
- **Handed to**: next session — start by reading this file + `00-DESIGN.md`. The recommended next cluster is **5.1 — Chat UX**: `OnboardingGate` in the (app) layout that redirects to `/onboarding` when `state.completedAt === null`; `/onboarding` page; `ChatSurface` client component that calls `/api/dev-agent/run-agent` per turn; `ProgressRail` for the 8 topic milestones (identity → income → expenses → debts → assets → goals → risk → preferences); a "we had trouble reaching Mavis" banner when `res.fellBack === true`. The persistence is ready (Part B); the chat UI just needs to call `runAgent` and render the messages. After 5.1, the queue is: 5.2 wire dashboard to identity (every surface reads from materialized records; seed moves to "demo mode"); 5.3 ongoing advisor via Ollama (post-onboarding "ask me anything" surface, same `callAgent` interface, privacy-first). Mavis is live (verified on 2026-08-25 with real CFP-grade responses; credits added). `LLM_PROVIDER` env switch works: `mock` for deterministic smokes, `mavis` for production, `ollama` for the local advisor. When Mavis/Ollama errors, the L1 rules fallback (deterministic mock engine) fires automatically. `.env.local.example` is committed; the actual `.env.local` is gitignored. The design system is **Sovereign Monad / vessel (v6)**, superseding Component Oracle Terminal. `tsc --noEmit` is clean; 15 smokes green at 551/551 (smoke-onboarding-agent 80, smoke-period 46, smoke-sidebar 60, smoke-topbar 102, smoke-bottom-dock 70, smoke-goals 36, smoke-deprecated 42, smoke-glossary 32, smoke-rebalance 5, smoke-reset-seed 8, smoke-engine-toggle 7, smoke-vessel-feed 7, smoke-horizon-strip 14, smoke-alert-bay 22, smoke-visual-finish 20); `smoke-auth.mjs` pre-existing failing — unrelated; dev server on 127.0.0.1:3000.
