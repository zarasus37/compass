...
> Last update: 2026-08-26 (Cluster Vault 4.0 M2 ✅ DONE — USDC funding + on-chain balance shipped as commit `b7ea8d8`). The `[FUND] $X USDC` button on /vault transfers testnet USDC from the server-side signer to the deployed Safe via the ERC-20 `transfer(safe, amount)` flow; the `[REFRESH] BALANCE` button reads the Safe's on-chain USDC balance via viem and writes the denormalized cache on `VaultAccount.onChainUsdcBalanceCents`. The vault principal KPI cell on the status strip now carries a `[OK] LIVE` badge + an "on-chain $X.XX" sub line when the Safe is deployed. The two buttons are hidden in MOCK state (deploy first). 6-decimal USDC math lives in `src/lib/vault/funding.ts` (parseUsdcCentsToUnits, centsFromUsdcUnits, usdcTransferData, getOnChainUsdcBalance, fundSafeWithUsdc); the denormalized cache + idempotency-by-nonce live in `src/lib/vault/db.ts` (setOnChainBalance, recordFunded, findFundedByIdempotencyKey). Two new audit action types: `vault.funded`, `vault.balance_refreshed`. Two new schema columns on `VaultAccount`: `onChainUsdcBalanceCents Int @default(0)`, `onChainBalanceRefreshedAt DateTime?`. Env: `VAULT_FUND_MAX_CENTS` (default 100_000_00 = $10k per call). integration-vault 116/116 (was 96, +20 for M2), smoke-vault 71/71 (was 65, +6 for M2), adjacent smokes green, tsc clean. **Remaining M3/M4: M3 = real yield via Aave V3 on Base Sepolia; M4 = off-ramp gateway (or deferred).** Predecessors: Vault 4.0 M1 (`da71ae5`). Vault 3.5 (`7ea2a60`). Vault 3.1 (`8618077`). Vault 3.0 (`c3a4c15`). Vault 2.5 (`a43b3d8`). Vault 2.0 (`d9e3dc9`). dev server on 127.0.0.1:3000.
