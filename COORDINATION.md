...
> Last update: 2026-08-30 22:25 CDT (Cluster 7.8.1 — **Audit log retention cron (M8) — pure infra, no visible UI** — committed on top of `8f7b23b` (Cluster 7.8) + new cluster commit. A nightly job that calls `pruneAuditLog` for every user, with the same pattern as the existing vault cron. New `pruneAuditLogForAllUsers(opts?)` bulk helper in `src/lib/vault/audit-log-cron.ts` iterates every user and calls `pruneAuditLog(userId, ...)` for each; each user's prune is wrapped in try/catch so one user's failure doesn't abort the batch — the `usersFailed` count + per-user `error` field let the operator see exactly which users failed. Returns `{ ok, usersProcessed, usersFailed, totalRolledUp, totalDeleted, totalRollupRows, results, retentionDays, now }` (same shape pattern as the vault cron's response). New `POST /api/cron/audit-log-prune` endpoint (POST + GET delegate to the bulk helper; auth: same `CRON_SECRET` bearer pattern as `/api/cron/vault` — required in prod, skipped in dev). New `scripts/cron-audit-prune-dev.mjs` long-running poller, configurable via `AUDIT_LOG_PRUNE_POLL_MS` env (default 24h; override to 5-30s in dev for fast feedback); logs `no users due` when nothing to prune, `processed=N rolledUp=X deleted=Y` when work happened, `ERROR <userId>...` per failed user, plus a final shutdown line on Ctrl-C. New `cron:dev:audit` npm script. The middleware gained `/api/cron` in the public prefixes list — **a pre-existing bug where the dev scheduler was being silently redirected to `/login` is now fixed** (the script's try/catch + JSON parse error tolerance hid the bug for months; every dev poll silently failed). `.env.production.example` documents the optional `CRON_SECRET` env var (generate with `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`). No visible-UI payoff (pure infra); the user sees nothing new, but the live `AuditLog` table is now bounded at ~90 days per user. **Idempotency + concurrency-safety**: two cron processes firing simultaneously both call `pruneAuditLogForAllUsers`; the second one finds nothing to prune (everything is already in the rollup) and returns `totalRolledUp: 0` — no double-delete, no data loss. New `tests/smoke-cron-audit-log-prune.mjs` (30 checks) covers: POST + GET return 200 with the right shape; the smoke user appears in `results` with `status: "PRUNED"` + `rolledUp >= 2`; 3 in-window sentinels remain in the live table; 1 rollup row for `(100d, smoke.cron_prune.old)` with `count=2`; idempotent 2nd call returns `status: "NOOP"`; source-file checks for the bulk helper, route, dev script, npm script, and middleware fix. New `tests/integration-vault.mjs` Phase 4.0 M8 (23 checks) covers the same wiring at the integration level + POST/GET wire checks. The 3 pre-existing audit-log misses in 7.7 (date-TZ edge cases — UTC vs local comparison on `?from=<rowDate>`) are unchanged; they're a follow-on to fix in cluster 7.9. All 19 data-layer smokes + 13 UI smokes + integration-vault 226 + smoke-deploy = ~1,620 checks, ALL GREEN. tsc clean. Dev server live on testnet default. Predecessor: Cluster 7.8 (commit `8f7b23b`). Spec: `00-CLUSTER-7.8.1-AUDIT-LOG-CRON.md`. **The next cluster is `vercel.json` with the cron schedule** — the endpoint is callable from any cron system; the schedule (`0 3 * * *` recommended) is the only missing piece for production.)
>
>
> (Previous: 2026-08-30 20:30 CDT (Cluster 7.8 — **Audit log retention — small infra + visible UI** — committed on top of `ef0982a` (Cluster 7.7) + new cluster commit. **The live `AuditLog` table now holds only the last 90 days of events**; older events are aggregated into a new `AuditLogDailyRollup` table (one row per `(userId, dateKey, actionType)` with `count` + `failedCount`) by a `pruneAuditLog(userId, options)` function. The prune is idempotent (upsert with `count: { increment }`); a nightly cron is the next cluster (M8). New `src/lib/vault/audit-log.ts` exports `getRetentionDays()` (default 90, override via `AUDIT_LOG_RETENTION_DAYS` env) and `pruneAuditLog(userId, { retentionDays?, now? })` (returns `{ rolledUp, deleted, rollupRows, retentionDays }`). New `prisma/schema.prisma` `AuditLogDailyRollup` model with `@@unique([userId, dateKey, actionType])` and indexes on `(userId, dateKey)` + `(userId, actionType)`; cascade-delete on the `User` back-relation. New dev-only `POST /api/dev/audit-log-prune` (same `NODE_ENV` gate as `audit-log-write`; body is `{ retentionDays?, now? }`; returns the full result). `getAuditLogActivity` refactored to read from BOTH the live table AND the rollup — the 30-day strip is unchanged (within the live horizon); the new 365-day strip gets the first 90 days from live rows and the rest from the rollup. `ActivityStrip` is now dynamic: ≤30 days keeps the original 30-bar layout; 31-90 days widens the SVG to fit 90 daily bars at 7px each; >90 days downsamples to 90 buckets (each bucket spans ~N/90 days). The strip header follows the active window (`// 30-day shape` → `// 90-day shape` → `// 365-day shape`) and a new `data-window-days` testid is set on the strip div. `DateRangeBar` gained a 6th chip: **"Last 12 months"** (`daysBack: 365`); the page's new `computeActivityDays` helper derives the strip's window from the active `from`/`to` filter. The `?from` / `?to` URL contract from C7.7 is unchanged. The 4-cell headline strip stays UNFILTERED (per the 7.4 contract); only the activity strip + table follow the date range. New `tests/smoke-audit-log-retention.mjs` (44 checks) covers: prune correctness (3 in-window rows stay, 4 out-of-window rows roll up), rollup `count` + `failedCount` per `(dateKey, actionType)`, `vault.payment_failed` flagged correctly via the FAILED set, prune idempotency (2nd call is a no-op), re-insert increment (4 rows in → 2nd prune increments rollup `count` to 4), dev endpoint gating, 90/365-day view rendering, ActivityStrip dynamic geometry + `data-window-days` testid. New `tests/integration-vault.mjs` Phase 4.0 M7 (20 checks) verifies the Prisma model, table, unique index, dev endpoint, `pruneAuditLog` + `getRetentionDays` exports, 365d preset, dynamic strip, and the dev endpoint's `NODE_ENV` gate. All 18 data-layer smokes + 13 UI smokes + integration-vault 203 + smoke-deploy = ~1,590 checks, ALL GREEN. The 3 pre-existing audit-log misses in 7.7 (date-TZ edge cases — UTC vs local comparison on `?from=<rowDate>`) are unchanged from the pre-7.8 baseline; they're documented in the spec as a follow-on to fix in cluster 7.9. tsc clean. Dev server live on testnet default. Predecessor: Cluster 7.7 (commit `ef0982a`). Spec: `00-CLUSTER-7.8-AUDIT-LOG-RETENTION.md`. **The next cluster is the nightly cron for `pruneAuditLog`** — the function + dev endpoint are wired; the cron (per-user, daily) is a small add.)
