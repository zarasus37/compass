...
> Last update: 2026-08-26 (Cluster 5.3.1 ✅ DONE — Advisor v2 with the read-only tool set shipped as commit `269d4ff`). 7 read-only tools (`queryTransactions`, `queryEnvelopes`, `queryBills`, `queryDebts`, `queryGoals`, `simulatePaycheck`, `summarizeSpending`) + multi-round orchestrator (cap 3 rounds). LLM provider routing: `LLM_PROVIDER_ADVISOR` env var (default `ollama`, independent of `LLM_PROVIDER`) so Mavis can drive onboarding while Ollama drives the advisor. System prompt expanded with the 7-tool registry + 5 worked example patterns (lights spend, grocery avg, trip savings, spending cut, safe-to-spend) + a "when to call tools vs. answer from context" decision rule. Dev-only `/api/dev/advisor/test-tool` endpoint exposes the handlers for smoke testing. Advisor smoke: 78 checks (was 32). All 25 smokes green except pre-existing `smoke-auth.mjs` (not from this work). tsc clean. **Predecessor: Cluster 5.3 (commit `8eef08b`) — `/advisor` page, basic CFP-on-tap with the identity inlined in the system prompt, no tools, single LLM round per turn. Pre-predecessor: Cluster 5.2.6 (commit `e1e9066`) — DB-backed widget switch (6 widgets on Prisma).** Pre-existing tsc error in `src/app/(app)/vault/page.tsx:136` (Property 'acknowledged' missing) — not introduced by this work, owned by the vault cluster. dev server on 127.0.0.1:3000.

### Cluster 5.3.2 — onboarding depth (next up, same session)

Per user direction (2026-08-26): add **spending-habit extraction** (new saveSpendingHabits tool — tracks "where the user actually shops", not just the transactions), **investment-detail extraction** (deepen the 401k/IRA/HSA/brokerage breakdown — what funds, employer match, vesting), and **"why this matters" inline per question** (each question the agent asks should include a 1-sentence "we use this for X" so the user understands the value of answering). Update the CFP onboarding system prompt + add 1-2 new tools + smoke + commit + COORDINATION.md. Same fresh-session handoff contract as 5.3.1 — the smoke starts from a known state and finishes green.
