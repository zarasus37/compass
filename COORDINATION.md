...
> Last update: 2026-08-29 21:30 CDT (Cluster 7.4 — **Audit log viewer (visible UI)** — commit `ee405f8`. The audit log has been growing since Cluster 2.0 (auto-allocate) and is now auditable itself. New `/vault/audit` page (force-dynamic, server-rendered) surfaces every action the system has taken on the user's behalf in one reverse-chronological view. Sections: 4-cell headline strip (total / this week / most active type / last activity, with growth-oriented suggestion chips per the 2026-08-24 directive — `[FILTER →]`, `[VIEW FAILURES →]`, `[VIEW LATEST →]`); 30-day activity strip (pure SVG, sqrt-scaled bars, gold today tick + `// TODAY` label, neg-red for any failure day, dashed stubs for empty days); horizontal stacked type distribution (one segment per actionType, click to filter, stable per-type color from a 10-color vessel palette via djb2 hash — same color shows up in the table chip); type filter pills (All + one per type, capped at 12 with `+N more`) + prefix filter (one button per distinct prefix in the user's data — `vault.`, `auto_`, `plan_`, etc., hidden when only one prefix exists) + free-text `?q=` search; the table (newest first, default 50, `?take=200` for more, click `[SHOW JSON]` to expand the payload in a `<details>` block, type chip carries the type's stable color). New `vault.audit_log_viewed` event type, written on every page render (fire-and-forget AFTER the read, so the just-written event doesn't show in the same visit's table — the next visit will). `[AUDIT] →` chip on `/vault/preferences` PageHead actions slot + `// view full history → /vault/audit` link from `/vault/schedule`. URL contract (server-side filter): `?type=<exactActionType>`, `?prefix=<vault|auto|plan|...>`, `?q=<substring>`, `?take=<n>` (default 50, max 200). Combinations are AND. The 4-cell summary always shows the unfiltered total + this-week counts; the activity strip + type distribution + table all respect the filter. New file `src/lib/vault/audit-log.ts` (data layer: `getAuditLog`, `getAuditLogSummary`, `getDistinctActionTypes`, `getAuditLogActivity`, `getAuditLogPrefixes`, `parseAuditLogFilter`, `recordAuditLogViewed`, `colorForActionType`). The new actionType is added to the `recordVaultAudit` union in `db.ts` (TS-only, Prisma's column is `String`). New `tests/smoke-audit-log.mjs`: 35 checks (was 0, +35). The smoke is self-contained: it writes a `smoke.test_audit_event` sentinel row directly via the shared Prisma client so the page is guaranteed to have at least 3 rows to render regardless of what prior smokes left in the user's audit log. **All 30 smokes green via `pnpm smoke:all`** (~1,510 checks, was ~1,475). tsc clean. **Visible-UI; no engine, no DB schema, no new dependencies.** Predecessor: Cluster 7.3 commit `589634f`. Spec: `00-CLUSTER-7.4-AUDIT-LOG-VIEWER.md`. Out of scope (deferred): real-time updates (no SSE/WebSocket), audit log retention / archival (table grows forever — future cluster: `vault.audit_log_pruned` + a cron), per-action filtering (only the existing actionType filter for v1; future: date range, payload-key search), audit log export (CSV / JSON download), cross-user audit log (admin / household views).)
>
