...
> Last update: 2026-09-25 (Cluster 7.16 — Self-service change password; AWAITING visual verification on Vercel preview + merge-to-main to ship to mom). Adds `/settings/password` — auth-gated route (requireUser, `dynamic = 'force-dynamic'`) under the /(app) shell, renders `<ChangePasswordForm />` inside the Oracle Terminal PageHead (`// system · settings · password` eyebrow). Three password fields (Current · New · Confirm New) with per-field error rendering + top error + success card on completion. New row `02.5` (Change Password, glyph ⚿, `[OK] SELF-SERVICE` tag) wired into the `/settings` hub. Server action `changePasswordAction` (`"use server"`) Zod-validates the input (12-char min, new !== old, new === confirm), verifies the old password with argon2id (`@node-rs/argon2`) before mutating, then runs the rotation as a single `prisma.$transaction { user.update + session.deleteMany + createSession }` so passwordHash + "log out everywhere else" + fresh session for the current device are atomic. **Cookie write happens AFTER the transaction commits** — a rollback never leaves a dead cookie pointing at a destroyed session row. Smoke harness updated: `tests/smoke-change-password.mjs` (new, 33 OK / 0 miss / 6 SKIP-NO-SERVER on this Windows box); `tests/integration-vault.mjs` Phase 4.0 M14 (16 wire checks, 22 OK / 0 miss on this Windows box via the source-only path). Both tests use a **server-probe + `[SKIP-NO-SERVER]`** pattern that adapts to the Windows + Turbopack PostCSS subprocess timeout — DB + source-file checks run end-to-end locally; HTTP-only checks run on Vercel preview. **Cluster 7.16 + Cluster 7.15 are both on `cluster/7.15-sparkline` HEAD `4ccb4e1`** and **READY on Vercel preview** at https://compass-j2rzvl3sq-sovereign-monad-ecosystem.vercel.app. Visual verification of the change-password surface (login → /settings → row 02.5 → submit form) is the next operator action; merge-to-main ships both clusters to mom. `pnpm tsc --noEmit` clean. Spec: `00-CLUSTER-7.16-CHANGE-PASSWORD.md`.
>
> (Previous: 2026-09-25 (Cluster 7.15 — Per-bill payment history sparkline). Adds a chart-first `// rhythm` strip between BillSummaryStrip and BillTimeline on /vault/bills/[id]/history — one colored dot per audit event, positioned by time, colored by tone (good/watch/bad/neutral via the C7.11.1 TONE_COLOR map). Hover shows actionType + humanized summary; click smooth-scrolls to the matching row in the LiveBillEventTable and gives it a 1.5s flash. Empty state + >80-event day-binning + same-timestamp vertical stacking handled. New component `src/app/(app)/vault/bills/[id]/history/_components/PaymentHistorySparkline.tsx` (client). `BillEventTableView` rows carry `id={r.id}` for the click-anchor. `tests/smoke-bill-history.mjs` grew to §28 (5 new sections, all green by source-check); `tests/integration-vault.mjs` Phase 4.0 M13 grew by 16 source + wire checks. `pnpm tsc --noEmit` clean. **No schema, no env, no middleware, no breaking changes** — pure visible-UI on an existing surface. **Local smoke run was blocked by pre-existing Tailwind v4 + shadcn@4.18.0 CSS incompatibility** (see HANDOVER §Cluster 7.15 for the diagnosis + the workaround applied to node_modules/shadcn/dist/tailwind.css). Runtime verification falls to the Vercel preview URL. Cluster 7.16 (mom-ready v1 launch) remains the live production posture.
>
> (Previous: 2026-08-31 07:00 CDT (Cluster 7.14 — per-bill off-ramp provider override UI + resolveChain fix). The cluster starts with a bug fix: `gateway.resolveChain` read `bill.providerPreference` (true) but the `adapters` Map is keyed by display names ("Mock" | "Spritz" | "Monto" | "Manual Push") while `OffRampProvider` is the enum form ("MOCK" | "SPRITZ" | "MONTO"). The bridge is `OFFRAMP_PROVIDER_ADAPTER_NAME`; `buildDefault` applied it, `resolveChain` didn't. A per-bill override written as the enum silently fell back to the user default — a "control that lies." The fix is a 3-line normalize via `resolveProviderAdapterName` (handles enum form, display form, and lowercase legacy form defensively). On top of that, the cluster ships the **per-bill off-ramp picker UI** that the C7.3 spec explicitly deferred as "future cluster": a 4-chip picker (`USE MY DEFAULT` + MOCK + Spritz + Monto) on `/vault/bills/[id]/history`, with the **resolved chain display** (Spritz → Mock → Monto → Manual Push) underneath — the user's "what happens if this fails?" question gets a literal visual answer. New server action `setBillProviderPreferenceAction` validates bill ownership (`findFirst({ vault: { userId } })` not `findUnique`), validates the provider against the union, writes a `vault.off_ramp_provider_changed` audit row with `{ scope: "bill", billId, from, to }`, revalidates the bill page + /vault + /obligations, and short-circuits no-ops. New per-bill chip on `/vault` (`Provider · Spritz` in gold) renders only when the override DIFFERS from the user default (silently inherits otherwise — one source of truth). `BILL_AUDITABLE_ACTION_TYPES` extended with `vault.off_ramp_provider_changed` so the per-bill history page renders the override-change timeline. New `tests/smoke-bill-provider-override.mjs` (39 checks) includes the **round-trip assertion** — set "SPRITZ", assert the rendered `data-chain` starts with "Spritz" — the assertion that wouldn't have existed before the fix. `tests/integration-vault.mjs` Phase 4.0 M12 added 20 source + wire checks. All 21 data-layer smokes (~1,150) + integration-vault 330 + smoke-deploy 102 = ~1,580 checks, ALL GREEN. tsc clean. No schema change, no new env vars, no middleware change. Predecessor: `8b13660` (Cluster 7.11.1). Spec: `00-CLUSTER-7.14-PER-BILL-OFF-RAMP-OVERRIDE.md` (renumbered from 7.12 after Polar flagged that 7.12 + 7.13 are taken in Drive).
>
>
> (Previous: 2026-08-31 00:50 CDT (Cluster 7.9 — **Audit log smoke fix — pure test, no user-visible change** — committed on top of `52bb94c` (Cluster 7.8.2) + new cluster commit. **The C7.7 dimming test was obsolete after C7.8's behavior change.** The C7.7-era test was: `GET /vault/audit?from=<5d-ago>&to=<today>` → expect ≥20 out-of-range bars. Pre-C7.8 the strip was always 30 days regardless of the filter, so a 5-day filter correctly dimmed 25 bars. C7.8 changed the strip to scale with the range — a `from+to` filter makes the strip exactly the window (all in-range, none out-of-range). The test's premise no longer held; it was failing in the post-C7.8 era. **The fix: use a from-only request** — `GET /vault/audit?from=<5d-ago>` (no `to`) keeps the strip at its 30-day default (per `computeActivityDays`: if either `from` or `to` is missing, return 30) while the filter is open-ended. The 25 bars before `5d-ago` are out-of-range; the 5 bars from `5d-ago` to today are in-range. Both `inRange >= 1` and `outOfRange >= 20` pass. The smoke's dimming logic is now exercised correctly post-C7.8. **No dimming feature change** — the dimming itself is unchanged; only the smoke's URL pattern was updated to a from-only query that exercises the dimming. **The two date-TZ misses in the same smoke resolved naturally.** Tests (1) `?from=<rowDate> includes the sentinel row` and (2) `?from + ?type composes (AND)` were failing when the smoke ran when local < UTC date (e.g., 22:00 CDT = 03:00 UTC the next day). At those times, the sentinel's UTC date was one day ahead of the local date, and `?from=<rowDate>` (which uses local-tz dates) excluded the sentinel. The cluster (7.9) was scoped to fix the dimming test (3) only; the date-TZ issues (1) and (2) resolved on their own when the clock drifted into a TZ where local and UTC dates align. They're a latent flakiness issue, not a C7.8 regression — a follow-on cluster could pin the smoke's `now` to noon local if the flakiness becomes a real problem. **The baseline is back to 100% green.** 19 data-layer smokes (915 checks) + integration-vault 239 + smoke-deploy 102 = ~1,680 checks, ALL GREEN. tsc clean. Predecessor: Cluster 7.8.2 (commit `52bb94c`). Spec: `00-CLUSTER-7.9-AUDIT-LOG-SMOKE-FIX.md`.)
> (Previous: 2026-08-29 15:55 CDT (Cluster 7.3 — **Off-ramp picker + MOCK adapter + Spritz sandbox-ready wiring (visible UI)** — commit `589634f`. The vault's off-ramp gateway finally has a user-level picker. New `VaultPreferences.offRampProvider` field (Prisma String column, default `MOCK`). New `OffRampProvider` TS union + `OFFRAMP_PROVIDER_LABEL` / `_DESC` / `_ADAPTER_NAME` maps in `src/lib/vault/types.ts`. New `MockOffRampAdapter` (clean success, no external call) in `adapters.ts`. New `SpritzClientAdapter` + `createSpritzAdapter({ userId })` factory in the new `src/lib/vault/spritz-client.ts` — env-gated on `SPRITZ_INTEGRATION_KEY` + `SPRITZ_SANDBOX=true`; missing env returns a `MockOffRampAdapter("Spritz")` so the chain stays end-to-end functional. `OffRampGateway.buildDefault(userId, preference)` now takes the user's preference and builds the chain with it first (other real adapters in canonical order, Manual Push terminal). `setOffRampProviderAction` server action validates the input + writes a `vault.off_ramp_provider_changed` audit row with `{ from, to }`. New `buildGatewayForUser(userId)` helper in `server.ts` is the single read point for the preference; `executeBillPaymentAction` + `retryBillPaymentAction` use it. New `OffRampProviderPicker` client component (mirrors `YieldRoutingPicker` pattern) on `/vault/preferences` with 3 chips (MOCK / Spritz / Monto). New `[PROVIDER] <name>` chip in `/vault` PageHead `actions` slot. `/vault` OffRampPanel grows to 4 rows (Mock / Spritz / Monto / Manual Push) and highlights the active provider with a `// CURRENT` chip + vessel-accent left border. The chain summary reads `<configured> → others → Manual Push` (no longer hardcoded). `PolicySummaryCard` grows to 5 cells; the new "off-ramp" cell shows the configured provider + a `[CONFIGURE]` link. `OffRampAdapterStatus` gets an `isActive: boolean` field; the snapshot projection computes it from the preference. `@spritz-finance/api-client ^0.11.0` added to package.json. **All 29 smokes green via `pnpm smoke:all`** (~1,470 checks; the new `smoke-off-ramp-picker` is 35 checks, `smoke-vault-prefs` is 66 — was 65, +1 for the 5th policy-summary cell). tsc clean. **Visible-UI + the infra that makes it functional; new SDK dep; default MOCK keeps end-to-end flows green without Spritz creds.** Predecessor: Cluster 7.2 (`29bc14f`). Spec: `00-CLUSTER-7.3-OFF-RAMP-PICKER.md`. Out of scope (deferred): real Spritz sandbox creds (xKryptic signs up at sdk.spritz.finance; code is wired, smoke covers the MOCK fallback), Monto real integration, real fiat bank-account linking, multi-rail failover, refund / dispute flow, per-bill UI override of the preference, audit-log UI for the new `vault.off_ramp_provider_changed` event.)
>
> (Older: 2026-08-29 05:45 CDT — Cluster 7.1 — **⌘K command palette (visible UI)** — commit `d71c224`. Same OffRampGateway + Aave V3 supply/withdraw flow as Cluster 6.0/M1-M4, but pointed at Base mainnet. Replaced the hardcoded `baseSepolia.id` check in `src/lib/vault/safe-deploy.ts` with a `CHAIN_TABLE` indexed by chainId; mirrored as `AAVE_CHAIN_TABLE` in `aave.ts`. Canonical mainnet addresses cross-referenced against the Aave address book + safe-deployments: Aave V3 Pool `0xa238dd80c259a72e81d7e4664a9801593f98d1c5`, Circle USDC `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913`, Safe singleton v1.3.0 `0x69f4D1788e39c87893C980c06EdF4b7f686e2938`. aUSDC stays dynamic via `Pool.getReserveData`. New read-only `GET /api/vault/chain-config` endpoint returns the resolved config; whitelisted in middleware. `/vault` StatusStrip now surfaces a gold `MAINNET` chip on chainId 8453 (priority over green LIVE). `prod.ts` refuses to start in production with `VAULT_CHAIN_ID=84532` (testnet). `.env.production.example` documents both modes — mainnet block is the live default, testnet block is a comment. **All 26 smokes green via `pnpm smoke:all`** (1 auth + 10 data-layer + 13 UI + 1 integration-vault + 1 deploy = 26; ~1,300 checks; smoke-deploy went from 72 to 95 checks). tsc clean. Dev server live on 127.0.0.1:3000 (currently on testnet default; flip `VAULT_CHAIN_ID=8453` in `.env.local` to target mainnet). Predecessor: Cluster 6.0 (commit `62b8528`). Spec: `00-CLUSTER-6.0.1-MAINNET-VAULT.md`. **Real mainnet deploy is still a manual gate** — the deployer EOA needs real ETH on Base, the [DEPLOY] button walks the same path the testnet does, but a wrong address or a misconfigured signer loses principal. No real mainnet deploy was performed in this cluster.
>
> **Postgres-everywhere**: SQLite → Postgres (Dev = Docker compose at `localhost:5433`; Prod = Neon/Supabase/RDS via `DATABASE_URL` with `?sslmode=require`). Prisma 7 + `@prisma/adapter-pg` over `pg`. New `tests/_smoke_db.mjs` is the single source of truth for the smoke client; every data-layer smoke imports it instead of rolling its own. `src/server/db.ts` swapped from `PrismaBetterSqlite3` to `PrismaPg`. Schema switched via `prisma db push --accept-data-loss`; no migration history yet (production should run `prisma migrate dev` once to seed `_prisma_migrations`).
>
> **Deploy hardening**: (1) `next.config.ts` now ships CSP + HSTS + X-Frame-Options + X-Content-Type-Options + Referrer-Policy + Permissions-Policy headers. (2) `src/lib/env/prod.ts` + root `instrumentation.ts` validate the production env at boot — refuses to start with the MOCK signer, the dev `LLM_PROVIDER=mock`, or the dev `DATABASE_URL`. (3) `GET /api/health` returns a 4-subsystem dashboard (`db` + `ai` + `env` + `vault`) with a `migrationStatus` field; the prod / dev response shape is the same, only the gating differs. (4) `/vault` shows a `[WARN] Testnet` banner whenever `VAULT_CHAIN_ID` (or `vault.chainId`) is a testnet; auto-hides on mainnet. (5) `.env.production.example` is the contract for the prod secret store. (6) `.github/workflows/ci.yml` runs `tsc + smoke + smoke:integration + smoke:deploy` on every PR with a Postgres service container. (7) `package.json` got `db:up/db:down/db:reset/db:push/db:generate/tsc/smoke/smoke:ui/smoke:integration/smoke:all/smoke:deploy` scripts. (8) `tests/smoke-deploy.mjs` is 55 deploy-readiness checks (env files, scripts, Prisma config, smoke-helper wiring, headers, docker, vault banner, health endpoint).
>
> **Gotcha to remember**: Windows host has native **PostgreSQL 18** on `5432` (installed via `postgresql-x64-18` service). Docker container is published on host port **5433** to avoid the collision. `DATABASE_URL` defaults to `postgresql://compass:compass@localhost:5433/compass_dev` everywhere. Production Postgres (Neon/Supabase/RDS) uses the default `5432`.
>
> The earlier 4 tech-debt items flagged "before a live account test" are all still resolved: (1) `tsc` clean (still), (2) `smoke-auth` green, (3) 0 terminal-token usages in app pages, (4) deprecated files unreachable. **24/24 + 1 deploy smoke = 25/25 green.**
...
- **From session**: `mvs_e751272c75984f30b387ef1c4ef0580f` (Cluster Vault 4.0 M2 — USDC funding + on-chain balance read. New `src/lib/vault/funding.ts` (USDC math + on-chain reads + funding flow + idempotency). 2 new schema columns on `VaultAccount`: `onChainUsdcBalanceCents`, `onChainBalanceRefreshedAt` (pushed via `prisma db push` + client regenerated; dev server restarted). 3 new db accessors: `setOnChainBalance`, `recordFunded`, `findFundedByIdempotencyKey`. 2 new audit action types: `vault.funded`, `vault.balance_refreshed`. 2 new server actions: `fundSafeAction`, `refreshSafeBalanceAction`. 2 new client components: `FundSafeButton` (preset chips $50/$100/$500/$1,000 + custom input + per-click nonce), `RefreshBalanceButton` (on-chain $X.XX + refreshed HH:MM:SS). /vault page wired the two buttons into the `VaultPauseRow` (deployed state only) and added a `[OK] LIVE` badge + on-chain sub line to the vault principal KPI cell. New env var `VAULT_FUND_MAX_CENTS` (default $10k per call). 116/116 integration-vault (was 96, +20 for M2), 71/71 smoke-vault (was 65, +6 for M2), tsc clean. All adjacent smokes (sidebar 63, deprecated 42, reset-seed 8) still green.)
- **Handed to**: next session — **Cluster Vault 4.0 M2 is ✅ DONE**. Recommended next cluster is **Vault 4.0 M3 — real yield via Aave V3 on Base Sepolia**: replace the `aave` yield adapter's hardcoded 4.25% with a real read of the Aave V3 Pool's `currentLiquidityRate` for USDC on Base Sepolia; have the Safe deposit USDC into Aave V3 via a new Safe transaction (`pool.supply(...)` on the Aave V3 Pool contract on Base Sepolia); the 4-strategy yield routing still works (the Safe is the actor for `APPLY_TO_NEXT_BILL` / `SPLIT_BY_ENVELOPE`). The M3 entry point: M2's funding flow is the input — the signer holds testnet USDC, click `[FUND] $X USDC` to move it into the Safe, then M3 deposits into Aave. The current background dev-server task may have been reaped at the 30-min cap; the Next process itself is independent. `.env.local.example` is committed; the actual `.env.local` is gitignored. The design system is **Sovereign Monad / vessel (v6)**. `tsc --noEmit` is clean; vault smokes green at 116 + 71 = 187 total checks; `smoke-auth.mjs` pre-existing failing — unrelated.
...
- `8618077` **Cluster Vault 3.1** — Yield routing strategies actually do something. New `appliedYieldCents` column on `Bill`; new `getNextExecutableBill` + `routeYieldForStrategy` (the 4-strategy dispatcher) in `db.ts`; `refreshVaultApyAction` now routes the accrued yield per the user's preference. New audit action type `vault.yield_routed`. 63/63 integration + 53/53 smoke green. tsc clean.
...
- **(m2 commit hash — see `git log --oneline -1` after this push)** **Cluster Vault 4.0 M2** — USDC funding + on-chain balance read. The `[FUND] $X USDC` button on /vault transfers testnet USDC from the server-side signer to the deployed Safe (ERC-20 `transfer(safe, amount)` via viem's `encodeFunctionData`); the `[REFRESH] BALANCE` button reads the Safe's on-chain USDC balance via viem and persists the denormalized cache on `VaultAccount.onChainUsdcBalanceCents`. The two buttons are hidden in MOCK state (deploy first). Pre-flight checks the signer has enough USDC; the per-call amount is bounded by `VAULT_FUND_MAX_CENTS` (default $10k). Idempotency: a per-click nonce the client generates; re-submit with the same nonce short-circuits to the prior audit row. Two new schema columns on `VaultAccount` (`onChainUsdcBalanceCents Int @default(0)`, `onChainBalanceRefreshedAt DateTime?`); new USDC math + on-chain reads + funding flow in `src/lib/vault/funding.ts`; 3 new db accessors (`setOnChainBalance`, `recordFunded`, `findFundedByIdempotencyKey`); 2 new server actions (`fundSafeAction`, `refreshSafeBalanceAction`); 2 new client components (`FundSafeButton` with preset chips $50/$100/$500/$1,000 + custom input, `RefreshBalanceButton` with `// on-chain: $X.XX USDC · refreshed HH:MM:SS`). 116/116 integration + 71/71 smoke green. tsc clean.
- `b7ea8d8` **Cluster Vault 4.0 M2** — USDC funding + on-chain balance read (replaces the placeholder entry above; full description in the body of this file). New `src/lib/vault/funding.ts` (~250 lines: 6-decimal USDC math, ERC-20 transfer encoding, on-chain reads, funding flow with pre-flight, idempotency helper). New schema columns on `VaultAccount` (`onChainUsdcBalanceCents`, `onChainBalanceRefreshedAt`). New db accessors (`setOnChainBalance`, `recordFunded`, `findFundedByIdempotencyKey`). New server actions (`fundSafeAction`, `refreshSafeBalanceAction`). New client components (`FundSafeButton` with preset chips + custom input, `RefreshBalanceButton` with refreshed-at line). Wired into `VaultPauseRow` (deployed state only) + `[OK] LIVE` badge on the vault principal KPI cell. New env var `VAULT_FUND_MAX_CENTS`. 116/116 integration-vault (was 96), 71/71 smoke-vault (was 65), tsc clean. Recommended next: **Vault 4.0 M3 — real yield via Aave V3 on Base Sepolia.**
...
## HANDOFF — Vault 4.0 M2 (next session)

**From session** `mvs_e751272c75984f30b387ef1c4ef0580f` **(2026-08-26, ~2h focused)**: shipped one commit for the vault.

- **Cluster Vault 4.0 M2** — USDC funding + on-chain balance read. The `[FUND] $X USDC` button on /vault transfers testnet USDC from the server-side signer to the deployed Safe (ERC-20 `transfer(safe, amount)` via viem); the `[REFRESH] BALANCE` button reads the Safe's on-chain USDC balance via viem and persists the denormalized cache on `VaultAccount.onChainUsdcBalanceCents`. The two buttons are hidden in MOCK state (deploy first). Pre-flight checks the signer has enough USDC; the per-call amount is bounded by `VAULT_FUND_MAX_CENTS` (default $10k). Idempotency: a per-click nonce the client generates; re-submit with the same nonce short-circuits to the prior audit row. 116/116 integration-vault, 71/71 smoke-vault, tsc clean.

**What just shipped (M2 surface, in detail)**

- **`VaultAccount.onChainUsdcBalanceCents Int @default(0)` + `onChainBalanceRefreshedAt DateTime?`** — the two new schema columns. Pushed via `npx prisma db push` + client regenerated. The Next dev server was restarted after the regen (HMR alone isn't enough — Next caches the generated client at the module-graph level; this is the same constraint as the PayPeriod / SystemSettings / GoalType / Bill / VaultPreferences introductions).
- **`src/lib/vault/funding.ts`** (new, ~250 lines) — the server-only USDC + on-chain lib. Exports:
  - `parseUsdcCentsToUnits(cents)` + `centsFromUsdcUnits(units)` — 6-decimal USDC math. `centsFromUsdcUnits` truncates sub-cent balances to $0.00; the truncation is intentional (we don't display fractions of a cent).
  - `usdcTransferData(to, amountUnits)` — viem's `encodeFunctionData` for the ERC-20 `transfer` function. Goes through viem rather than hand-rolling the selector so a future ABI bump doesn't need a rewrite.
  - `getOnChainUsdcBalance(safeAddress)` + `getOnChainNativeBalance(safeAddress)` — viem `readContract` for `balanceOf(safe)`. The native balance returns `0n` on RPC failure (the off-ramp keeper can re-check later).
  - `fundSafeWithUsdc({ safeAddress, amountCents })` — the funding flow: pre-flight (signer USDC balance) → encode + broadcast transfer → wait for receipt (120s) → read post-balance for a sanity check. Throws on MOCK / invalid amount / RPC down / insufficient USDC / reverted receipt.
  - `fundingIdempotencyKey({ vaultId, amountCents, nonce })` — re-exported for the server action's idempotency check.
- **`src/lib/vault/db.ts`** — 3 new accessors + 2 new audit action types:
  - `setOnChainBalance({ vaultId, onChainUsdcBalanceCents, refreshedAt })` — updates the denormalized cache. Does NOT touch `availableBalance` (the simulated sum of envelope principal) — the two are distinct views, surfaced separately on the page.
  - `recordFunded({...})` — writes `vault.funded` with the full funding context (tx hash, amount, from/to addresses, nonce, post-balance).
  - `findFundedByIdempotencyKey(userId, nonce)` — looks up a prior `vault.funded` row by its nonce. Used by `fundSafeAction` to short-circuit re-submits. Scans the most recent 25 rows (the typical double-click window); larger histories would warrant a proper indexed lookup.
  - `recordVaultAudit` action-type union extended with `vault.funded` and `vault.balance_refreshed`.
- **`src/lib/vault/server.ts`** — 2 new server actions:
  - `fundSafeAction(amountCents, nonce)` — validates amount (positive integer cents, within `VAULT_FUND_MAX_CENTS`), rejects when the Safe is MOCK, checks the idempotency nonce, broadcasts the transfer, persists the cache + audit. Audits the failure too (e.g. signer out of USDC) so the user has a record of why the broadcast didn't happen.
  - `refreshSafeBalanceAction()` — reads the on-chain USDC balance, persists the cache, writes `vault.balance_refreshed`. Returns a structured error on RPC failure so the button can surface a clear line.
- **`src/lib/vault/actions.ts`** — re-exports `fundSafeFromPage(amountCents, nonce)` + `refreshSafeBalanceFromPage()` for client components.
- **`src/lib/vault/types.ts`** + **`src/lib/vault/mock-data.ts`** — `VaultAccount` + `VaultSnapshot.kpis` extended with `onChainUsdcBalanceCents` and `onChainBalanceRefreshedAt`. The in-memory mock returns `0` / `null` (no chain); the DB-sourced path reads from the column.
- **`src/components/vault/FundSafeButton.tsx`** (new) — the `[FUND] $X USDC` CTA. Preset chips ($50, $100, $500, $1,000) + custom input (dollars → cents conversion on submit). Generates a per-click nonce via `formatNonce()`. Pending / error / success states surfaced inline. The `useTransition` + `router.refresh()` pattern matches the existing CTAs (Deploy / Refresh APY).
- **`src/components/vault/RefreshBalanceButton.tsx`** (new) — the `[REFRESH] BALANCE` CTA. Shows `// on-chain: $X.XX USDC · refreshed HH:MM:SS` next to the button. Pending / error states inline. The success path updates `lastOk` + `lastBalance` locally so the new timestamp is visible without a network round-trip; `router.refresh()` then revalidates the server-rendered view.
- **`src/app/(app)/vault/page.tsx`** — wired the two buttons into the `VaultPauseRow`, conditional on `isMockSafeAddress(vault.smartAccountAddress)` (deployed state only). The 5-cell status strip's "vault principal" cell now shows an "on-chain $X.XX" sub line + a `[OK] LIVE` badge in deployed state. The `VaultPauseRow`'s control label also picks up a `[OK] SAFE DEPLOYED` suffix in deployed state.
- **`.env.local.example`** — added `VAULT_FUND_MAX_CENTS` (default 100_000_00 = $10k per call). The min is hard-coded to $1 in the server action (below 1 cent risks being a typo).
- **`tests/integration-vault.mjs`** — +20 checks (now 116/116). The M2 block:
  - Schema: `onChainUsdcBalanceCents` + `onChainBalanceRefreshedAt` columns present, default 0 / null in MOCK.
  - MOCK-state page surface: the [FUND] / [REFRESH] BALANCE / `[OK] SAFE DEPLOYED` status / `[OK] LIVE` badge are all hidden. The "on-chain $" sub line is NOT in the cell.
  - Deployed-state page surface (simulated by writing the real Safe + signer + chainId + on-chain balance to the DB + audit log): all the M2 markers are PRESENT. The `[FUND]` button shows the `$100.00 USDC` label when the $100 preset is active. The `[REFRESH] BALANCE` button shows `// on-chain: $100.00 USDC · refreshed HH:MM:SS`.
  - Audit: `vault.balance_refreshed` and `vault.funded` rows are writable + the nonce-based idempotency lookup matches.
  - Reset: the test restores the MOCK state + drops the simulated M2 audit rows so the next run starts clean.
- **`tests/smoke-vault.mjs`** — +6 checks (now 71/71). The M2 block:
  - In MOCK state: `[FUND]` + `[REFRESH] BALANCE` + `[OK] SAFE DEPLOYED` + `[OK] LIVE` badge are absent. The "on-chain $" sub line is NOT in the cell.
  - The yield-attribution `[SYNC] REFRESH APY` button (Phase 3.0) is still visible in MOCK state — the yield adapter is independent of the Safe deploy.

**Where Compass is right now** (smoke summary)

- integration-vault: **116/116** · smoke-vault: **71/71** · smoke-sidebar: 63/63 · smoke-reset-seed: 8/8 · smoke-deprecated: 42/42 · `tsc --noEmit` clean.
- Untested in this session (unchanged from the prior baseline): smoke-auth (pre-existing fail, not from this work), smoke-alert-bay, smoke-bottom-dock, smoke-engine-toggle, smoke-glossary, smoke-goals, smoke-horizon-strip, smoke-onboarding-agent, smoke-period, smoke-rebalance, smoke-topbar, smoke-vessel-feed, smoke-visual-finish.
- The 5.2.6 widgets 1-6 work is committed (sign-off in the body of this file).
- Dev server on `127.0.0.1:3000`. Restart pattern after any schema change: kill the Next process, then `npx next dev -p 3000` in a new background task. The background-task `bg_1591df47-0d23-4516-b107-4cb648004208` was the latest M2-session dev server (may have been reaped at the 30-min cap; the Next process itself is independent).

**Recommended next cluster (Vault 4.0 M3)**

- **Real yield via Aave V3 on Base Sepolia** — the yield adapter (Phase 3.0 stub) reads from `VAULT_YIELD_ADAPTER=mock|sky|aave`; M3 replaces `aave` with a real read of the Aave V3 Pool's `currentLiquidityRate` for USDC on Base Sepolia. The Safe deposits USDC into Aave V3 via a new Safe transaction (`pool.supply(...)` on the Aave V3 Pool contract on Base Sepolia). The 4-strategy yield routing still works (the Safe is the actor for `APPLY_TO_NEXT_BILL` / `SPLIT_BY_ENVELOPE`).
- **Out-of-band setup the user needs to do before M3 ships**: get some testnet USDC into the Safe first (M2 just shipped for that), then approve the Aave V3 Pool to spend the Safe's USDC, then call `pool.supply(...)`. The first $1 of yield is the happy-path test.
- **Optional follow-ups before M3**:
  - **Soft delete for bills** — replace the 3.5 hard delete with an `isArchived` flag (UI + DB) so users can recover an accidentally deleted bill.
  - **Bulk import** — CSV / "paste a list" to seed many bills at once (handy for the closed beta; the single-bill form is fine for one-off adds).
  - **`/obligations/new` wiring** — route the existing on-period bills form at `/obligations/new` to `createBillFromPage` so a user adding a bill from the periods page lands it in the vault automatically.

**How to pick up**

1. `git log --oneline -3` to see the new M2 commit.
2. Read this handoff + the M1 handoff above for the deploy + funding contract + the env-var contract.
3. The signer for M3 = the same EOA used for M1 deploy + M2 fund. Make sure it holds both testnet ETH (for gas on the deposit tx) AND testnet USDC (to deposit into Aave V3). M2's funding flow is the input for M3.
4. Sign in as `mom@compass.local` / `correct-horse-battery-staple`; visit `/vault`. Click `[DEPLOY] Safe` (if not already deployed) → `[FUND] $100 USDC` (or any preset) → `[REFRESH] BALANCE` — the on-chain balance should match. Then click `[SYNC] REFRESH APY` to verify the yield adapter still runs. The audit log gets `vault.funded` + `vault.balance_refreshed` + `vault.apy_refreshed` entries.
5. The dev server is on `127.0.0.1:3000`. The background task may have been reaped; start a fresh one with `npx next dev -p 3000` if needed.
6. The COORDINATION.md is committed; this handoff is the contract for Vault 4.0 M3.

---

## HANDOFF — Vault 4.0 M3 (next session)

**From session** `mvs_e751272c75984f30b387ef1c4ef0580f` **(2026-08-26, ~1h focused)**: shipped one commit for the vault.

- `ade8336` **Cluster Vault 4.0 M3** — real yield via Aave V3 on Base Sepolia (replaces the placeholder entry above; full description in the body of this file). New `src/lib/vault/aave.ts` (~700 lines: Aave V3 on-chain APY read from `Pool.getReserveData(USDC).currentLiquidityRate`, dynamic aToken resolution, USDC approve + supply + withdraw calldata builders, Protocol Kit `Safe.init` + `createTransaction` + `executeTransaction` wrapper). New `getReserveApy()` async function replaces the hardcoded 4.25% in `AaveAdapter.getCurrentApy()`; the `VAULT_AAVE_APY` env var is the silent fallback when the RPC is unreachable. New schema columns on `VaultAccount` (`onChainAUsdcBalanceCents`, `aUsdcBalanceRefreshedAt`, `aUsdcTokenAddress`). New db accessors (`setAUsdcBalance`, `recordAaveSupply`, `recordAaveWithdraw`, `findAaveSupplyByIdempotencyKey`, `findAaveWithdrawByIdempotencyKey`). New server actions (`depositSafeUsdcAction`, `withdrawSafeUsdcAction`, `refreshAUsdcBalanceAction`). New client components (`DepositButton` + `WithdrawButton`, both with preset chips + custom input + per-click nonce). Wired into `VaultPauseRow` (deployed state only) + "· aUSDC $X.XX earning" sub line on the vault principal KPI cell. New env vars `VAULT_AAVE_POOL_ADDRESS` (Aave V3 Pool on Base Sepolia), `VAULT_AAVE_USDC_ADDRESS` (Aave's faucet USDC, distinct from M2's Circle USDC), `VAULT_AAVE_AUSDC_ADDRESS` (fallback aToken), `VAULT_DEPOSIT_MAX_CENTS`. The integration test caught a real wiring bug on the first run (`DepositButton.onAaveUsdc` was wired to the M2 Circle USDC balance — wrong token); fixed in the same commit to use `vault.aUsdcTokenAddress != null` (a real signal: the Aave reserve is listed and the aToken is resolved). 144/144 integration-vault (was 116), 74/74 smoke-vault (was 71), tsc clean. Recommended next: M4 (off-ramp gateway) is **deferred to a later cluster** — see HANDOFF — Vault 4.0 M4 (deferred) below. The `aave` yield adapter now reads the real on-chain `currentLiquidityRate` from the Aave V3 Pool on Base Sepolia (replacing the hardcoded 4.25%). The Safe deposits USDC into Aave V3 via two new Safe transactions: `approve(usdc, pool, type(uint256).max)` on first supply (subsequent deposits skip the approval via a `getCurrentAllowance` pre-flight) and `pool.supply(usdc, amount, safe, 0)` which mints aUSDC to the Safe. The `[DEPOSIT] $X → AAVE` button on /vault calls `depositSafeUsdcAction`; the `[WITHDRAW] $X aUSDC` button calls `withdrawSafeUsdcAction` which burns aUSDC + sends USDC back to the Safe. Both buttons are hidden in MOCK state (deploy + fund first). The aUSDC balance surfaces on the status strip as "· aUSDC $X.XX earning" when > 0. 144/144 integration-vault, 74/74 smoke-vault, tsc clean.

**What just shipped (M3 surface, in detail)**

- **`VaultAccount.onChainAUsdcBalanceCents Int @default(0)` + `aUsdcBalanceRefreshedAt DateTime?` + `aUsdcTokenAddress String?`** — the 3 new schema columns. Pushed via `npx prisma db push` + client regenerated. The Next dev server was restarted after the regen (HMR alone isn't enough — Next caches the generated client at the module-graph level; same constraint as M2's two new columns).
- **`src/lib/vault/aave.ts`** (new, ~700 lines) — the server-only Aave V3 lib. Exports:
  - `getAaveChainConfig()` — reads the Aave env vars, returns the typed config (chainId, pool, usdc, aUSDC fallback). Deliberately excludes `rpcUrl` + `safeSingletonAddress` (those live on the base `ChainConfig` from `safe-deploy.ts`).
  - `getReserveApy(asset)` — the real on-chain APY read. Calls `Pool.getReserveData(asset).currentLiquidityRate` (in ray = 1e27, per-second), converts to APY via `(rateRay * SECONDS_PER_YEAR) / RAY / 1e18`. Clamps to [0, 1] for safety. Throws on RPC failure — the adapter catches and falls back to the `VAULT_AAVE_APY` env var (default 0.0425) so the page keeps rendering with last-known rate.
  - `getAUsdcTokenAddress()` — dynamic read from `Pool.getReserveData(USDC).aTokenAddress`. Returns the live aUSDC address for the configured Aave-USDC.
  - `getAaveUsdcBalance(safeAddress)` + `getAUsdcBalance(safeAddress, aUsdcAddress)` — viem `readContract` for `balanceOf(safe)` on the two tokens. Both return `0n` on RPC failure.
  - `getCurrentAllowance(safeAddress, usdcAddress, poolAddress)` — pre-flight to skip the approval tx on subsequent deposits.
  - `buildSupplyCalldata(asset, amount, onBehalfOf, referralCode)` + `buildWithdrawCalldata(asset, amount, to)` + `buildApproveCalldata(spender, amount)` — viem `encodeFunctionData` for the three Pool/ERC-20 calls. No hand-rolled selectors.
  - `executeSafeTransaction({ safeAddress, signerKey, chainConfig, to, data, value })` — Protocol Kit wrapper. `Safe.init({ provider, signerKey, safeAddress })` → `createTransaction` → `executeTransaction`. Returns the Safe-side tx hash.
  - `supplySafeUsdc({ safeAddress, signerKey, amountCents })` + `withdrawSafeUsdc({ safeAddress, signerKey, amountCents })` — the two flows. Pre-flight: read the Aave-USDC balance (and aUSDC balance for withdraw) → approval if needed (supply) → Safe.execTransaction → return `{ txHash, postBalanceCents, aUsdcAddress }`.
- **`src/lib/vault/safe-deploy.ts`** — exported the previously-internal `getSafeSignerKey()`. The Protocol Kit takes a raw `0x...` key string, not a viem `Account`, so this had to be public.
- **`src/lib/vault/yield-adapters.ts`** — `AaveAdapter.getCurrentApy()` is now `async` and returns the real on-chain APY via `getReserveApy()`. The `VAULT_AAVE_APY` env var is the silent fallback when the RPC is unreachable (RPC down, reserve unlisted, etc.) — the page keeps rendering with last-known rate, no console error.
- **`src/lib/vault/db.ts`** — 5 new accessors:
  - `setAUsdcBalance({ vaultId, onChainAUsdcBalanceCents, refreshedAt, aUsdcTokenAddress? })` — updates the denormalized aUSDC cache. Caches `aUsdcTokenAddress` on the row so subsequent reads skip the Pool hop.
  - `recordAaveSupply({...})` — writes `vault.aave_supply` with the full supply context (supply tx hash, optional approve tx hash, amount, aToken address, post-balance, nonce).
  - `recordAaveWithdraw({...})` — writes `vault.aave_withdraw` with the full withdraw context.
  - `findAaveSupplyByIdempotencyKey(userId, nonce)` + `findAaveWithdrawByIdempotencyKey(userId, nonce)` — look up prior `vault.aave_supply` / `vault.aave_withdraw` rows by nonce. Used by the server actions to short-circuit re-submits. Scans the most recent 25 rows.
  - `recordVaultAudit` action-type union extended with `vault.aave_supply` and `vault.aave_withdraw`.
- **`src/lib/vault/types.ts`** + **`src/lib/vault/mock-data.ts`** — `VaultAccount` + `VaultSnapshot.kpis` extended with `onChainAUsdcBalanceCents`, `aUsdcBalanceRefreshedAt`, `aUsdcTokenAddress?`. The in-memory mock returns `0` / `null` / `null` (no chain); the DB-sourced path reads from the columns.
- **`src/lib/vault/server.ts`** — 3 new server actions:
  - `depositSafeUsdcAction(amountCents, nonce)` — validates amount (positive integer cents, within `VAULT_DEPOSIT_MAX_CENTS`), rejects when the Safe is MOCK or the signer key is unset, checks the idempotency nonce, calls `supplySafeUsdc`, persists the aUSDC cache + writes `vault.aave_supply`. Audits the failure too (e.g. insufficient Aave-USDC) so the user has a record of why the supply didn't happen.
  - `withdrawSafeUsdcAction(amountCents, nonce)` — same shape, calls `withdrawSafeUsdc`, updates the cache + writes `vault.aave_withdraw`.
  - `refreshAUsdcBalanceAction()` — reads the on-chain aUSDC balance via `getAUsdcBalance`, persists the cache + writes `vault.balance_refreshed` with `kind: "ausdc"`. Returns a structured error on RPC failure so the button can surface a clear line.
  - Also `projectDbToLegacy` updated to wire the 2 new kpis fields.
- **`src/lib/vault/actions.ts`** — re-exports `depositSafeUsdcFromPage(amountCents, nonce)`, `withdrawSafeUsdcFromPage(amountCents, nonce)`, `refreshAUsdcBalanceFromPage()` for client components.
- **`src/components/vault/DepositButton.tsx`** (new) — the `[DEPOSIT] $X → AAVE` CTA. Preset chips ($50, $100, $500, $1,000) + custom input (dollars → cents conversion on submit). Generates a per-click nonce via `formatNonce()`. The `// aave-usdc: ready | needs faucet` status line above the chips; flips to "ready" when `vault.aUsdcTokenAddress != null` (a real signal: the Aave reserve is listed and the aToken is resolved; the on-chain pre-flight in the server action is the authoritative check). Pending / error / success states surfaced inline. `useTransition` + `router.refresh()` on success.
- **`src/components/vault/WithdrawButton.tsx`** (new) — the `[WITHDRAW] $X aUSDC` CTA. Same shape as Deposit. The `// aUSDC: $X.XX` status line above the chips shows the current aUSDC balance.
- **`src/app/(app)/vault/page.tsx`** — wired `DepositButton` + `WithdrawButton` into the `VaultPauseRow` (deployed state only), conditional on `isMockSafeAddress(vault.smartAccountAddress)`. The 5-cell status strip's "vault principal" cell now shows an additional "· aUSDC $X.XX earning" sub line when `kpis.onChainAUsdcBalanceCents > 0`. The `DepositButton`'s `onAaveUsdc` prop is wired to `vault.aUsdcTokenAddress != null` (the resolved aToken signal), not `kpis.onChainUsdcBalanceCents > 0` (that was the M2 Circle USDC balance — wrong token; caught by the integration test on the first run, fixed in the same commit).
- **`.env.local.example`** — added 4 new vars:
  - `VAULT_AAVE_POOL_ADDRESS` → `0x8bAB6d1b75f19e9eD9fCe8b9BD338844fF79aE27` (Aave V3 Pool on Base Sepolia)
  - `VAULT_AAVE_USDC_ADDRESS` → `0xba50Cd2A20f6DA35D788639E581bca8d0B5d4D5f` (Aave's faucet USDC on Base Sepolia; **distinct from M2's Circle USDC** `0x036CbD53842c5426634e7929541eC2318f3dCF7e`)
  - `VAULT_AAVE_AUSDC_ADDRESS` → `0x10F1A9D11cd9b7F9D46f8B6f8C5E2dF6e0c5F2a2C` (fallback aToken; resolved dynamically on first supply)
  - `VAULT_DEPOSIT_MAX_CENTS` (default 100_000_00 = $10k per call)
  - Also `VAULT_AAVE_APY` retained as the silent fallback when `getReserveApy()` throws (default 0.0425).
- **`tests/integration-vault.mjs`** — +28 checks (now 144/144). The M3 block:
  - Schema: 3 new aUSDC columns present, default 0 / null / null in MOCK.
  - MOCK-state page surface: `[DEPOSIT] AAVE` + `[WITHDRAW] AAVE` buttons hidden. The "aUSDC $" sub line is NOT in the cell.
  - Deployed-state page surface (simulated by writing the real Safe + signer + chainId to the DB + audit log): both buttons + their preset chips are PRESENT. The `// aave-usdc: needs faucet` line is shown (aUsdcTokenAddress is null at this point).
  - Supply simulation (write `onChainAUsdcBalanceCents` + `aUsdcBalanceRefreshedAt` + `aUsdcTokenAddress` + `vault.aave_supply` audit row): all 3 cache columns updated, the "aUSDC $50.00" sub line appears on the page, the `// aave-usdc: ready` line flips.
  - Withdraw simulation (write the post-withdraw aUSDC + USDC balances + `vault.aave_withdraw` audit row): both balances reflect the redemption, the "aUSDC $25.00" sub line shows the decremented value.
  - Audit: `vault.aave_supply` + `vault.aave_withdraw` rows are writable + the nonce-based idempotency lookups match. Unknown nonces return `null` (negative check).
  - Reset: the test restores the MOCK state + clears the 3 new aUSDC columns + drops the simulated M3 audit rows so the next run starts clean.
- **`tests/smoke-vault.mjs`** — +3 checks (now 74/74). The M3 block:
  - In MOCK state: `[DEPOSIT] AAVE` + `[WITHDRAW] AAVE` buttons are absent. The "aUSDC $" sub line is NOT in the cell.

**Out-of-band setup the user needs to do before clicking [DEPOSIT]**

1. The Safe must be deployed (M1) and funded with M2's Circle USDC.
2. **Mint Aave-USDC to the Safe** at Aave's faucet: `https://app.aave.com/faucet/`, select **Base Sepolia**, paste the Safe address, mint. This is a separate token from M2's Circle USDC — M2 funds with Circle's USDC; M3 deposits Aave's USDC.
3. After the first supply, the Aave aToken address is resolved and cached on the row. Subsequent supplies skip the Pool hop.

**Where Compass is right now** (smoke summary)

- integration-vault: **144/144** · smoke-vault: **74/74** · smoke-sidebar: 63/63 · smoke-reset-seed: 8/8 · smoke-deprecated: 42/42 · `tsc --noEmit` clean.
- Untested in this session (unchanged from the prior baseline): smoke-auth (pre-existing fail, not from this work), smoke-alert-bay, smoke-bottom-dock, smoke-engine-toggle, smoke-glossary, smoke-goals, smoke-horizon-strip, smoke-onboarding-agent, smoke-period, smoke-rebalance, smoke-topbar, smoke-vessel-feed, smoke-visual-finish.
- The 5.2.6 widgets 1-6 work is committed (sign-off in the body of this file).
- Dev server on `127.0.0.1:3000`. Restart pattern after any schema change: kill the Next process, then `npx next dev -p 3000` in a new background task.

**M3 contract for fresh-session pickup**

1. `git log --oneline -3` to see the new M3 commit.
2. Read this handoff + the M1 + M2 handoffs above for the deploy + funding + supply contract + the env-var contract.
3. The signer for M3 = the same EOA used for M1 deploy + M2 fund. Make sure it holds both testnet ETH (for gas on the supply + approve txs) AND testnet Aave-USDC (to deposit into Aave V3). M2's funding flow is the input — but the M2-funded Circle USDC is a different token; the M3 deposit needs Aave-USDC, which comes from Aave's faucet.
4. Sign in as `mom@compass.local` / `correct-horse-battery-staple`; visit `/vault`. Click `[DEPLOY] Safe` (if not already deployed) → mint Aave-USDC to the Safe at the Aave faucet → `[DEPOSIT] $50 → AAVE` (or any preset) — the supply broadcasts two txs (approve + supply), mints aUSDC to the Safe, the audit log gets `vault.aave_supply`. The aUSDC balance + sub line appear on the page. Then click `[WITHDRAW] $25 aUSDC` to burn aUSDC + send USDC back to the Safe.
5. The dev server is on `127.0.0.1:3000`. The background task may have been reaped; start a fresh one with `npx next dev -p 3000` if needed.
6. The COORDINATION.md is committed; this handoff is the contract for Vault 4.0 M4 (or whatever comes next).

---

## HANDOFF — Vault 4.0 M4 (deferred)

**Status**: M4 (off-ramp gateway) is **deferred to a later cluster**. The off-ramp panel on /vault still shows the Phase 1.0 stubs ("Phase 4 — would call Spritz here"). When M4 is picked up, the entry point is: the M2 funding flow gets USDC into the Safe; M3 deposits it into Aave V3 to earn yield; M4 withdraws the USDC (principal + yield) from Aave V3 and routes it through a real off-ramp provider (Spritz / Monto / etc.) to pay the bill. The Phase 1.0 13-state bill payment state machine + the Phase 2.5 transition menu + the Phase 3.0 audit log are all in place; M4 is the wiring that calls the real provider instead of simulating.

**Optional follow-ups before M4 ships (or any of them could become a new cluster)**:
- **Soft delete for bills** — replace the 3.5 hard delete with an `isArchived` flag (UI + DB) so users can recover an accidentally deleted bill.
- **Bulk import** — CSV / "paste a list" to seed many bills at once (handy for the closed beta; the single-bill form is fine for one-off adds).
- **`/obligations/new` wiring** — route the existing on-period bills form at `/obligations/new` to `createBillFromPage` so a user adding a bill from the periods page lands it in the vault automatically.
- **`_aave-supply-tx-hash` in the audit payload** — already there; just calling it out for grep-ability.
- **Replace the `vault.aave_supply` / `vault.aave_withdraw` "nonce lookup" with a real indexed column** — the current 25-row scan is fine for the double-click window, but a proper `(userId, actionType, payload->>'nonce')` index would be cleaner at scale. Postgres migration later is one-line swap.
