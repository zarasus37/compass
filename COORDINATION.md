...
> Last update: 2026-08-30 00:55 CDT (Cluster 7.5 — **Per-bill audit drill-down (visible UI)** — committed on top of `ed4129b` (handoff) + new cluster commit. The `/vault/audit` page (7.4) shows every event for the user, but when a `vault.payment_settled` or `vault.bill_state_changed` row catches the user's eye, the user has no way to see "everything that ever happened to this bill." New `/vault/bills/[id]/history` route (force-dynamic, server-rendered) mirrors the audit-log pattern but scoped to a single bill. Sections: BillHeader (bill name in Sora, vessel affiliation chip, amount in mono USD, frequency, current state badge with the right tone, last attempt, source chip, back-links to `/vault` + `/vault/audit?type=vault.bill_state_changed`); BillSummaryStrip (4 cells with growth-oriented suggestion chips per the 2026-08-24 directive — `[OPEN EVENT →]`, `[TIMELINE →]`, `[FILTER →]`); BillTimeline (visual-first state stepper — 5 happy-path states EARNING → FUNDED → PREPARING → EXECUTING → SETTLED as connected nodes, each with the count of `vault.bill_state_changed` rows that landed in that state, current state pulses, plus a row of 4 alternate-state badges (ACTION REQUIRED / PAY MANUALLY / PAUSED / CANCELLED) shown only if visited); BillEventTable (newest first, default 50, `?take=200` for more, click `[SHOW JSON]` for the payload, type chip uses the same `colorForActionType` palette from 7.4 so the same actionType shows the same color across both surfaces). URL contract (server-side filter, per-bill scope narrows to `?type=<exactActionType>` + `?take=<n>` only). New `vault.bill_history_viewed` event type, written on every render (fire-and-forget AFTER the read, so the just-written event doesn't show in the same visit's table — the next visit will). New 404 panel (not a hard 404) when the bill id doesn't exist for this user, so the surrounding chrome (PageHead, back-links) stays visible. Two deep-link wirings: (1) the `/vault/audit` table row's `// when` cell becomes a `<Link>` to the bill's history when `payload.billId` is set, with the `?type=` filter preserved for state-change + payment outcome rows (via the new `billHistoryHrefForAuditRow` helper in `audit-log.ts`); (2) the `/vault` bill list's bill name becomes a `<Link>` to the bill's history. New data layer: `src/lib/vault/audit-log.ts` adds `getBillByIdForUser`, `getBillAuditLog` (narrows the DB query to the 12 actionTypes that can carry a `billId`, then post-filters in JS over the parsed payload — also checks `billsCredited` for `vault.yield_routed` rows), `getBillAuditSummary` (4-cell numbers + `stateTransitionsByState` map for the timeline), `recordBillHistoryViewed`, `parseBillHistoryFilter`, `billHistoryFilterToQuery`, `billHistoryHrefForAuditRow`. New `vault.bill_history_viewed` event type added to the `recordVaultAudit` actionType union in `db.ts` (TS-only, Prisma's column is `String`). New smoke `tests/smoke-bill-history.mjs` — 46 checks (self-contained: writes a sentinel Envelope + VaultEnvelope + ScheduledBill via shared Prisma, then 3 sentinel audit events of distinct types, so the page is guaranteed to have data to render regardless of prior smoke state). `package.json` smoke script includes `smoke-bill-history.mjs` after `smoke-audit-log.mjs`. New "Phase 4.0 M5 — Bill audit drill-down" section in `tests/integration-vault.mjs` (8 new checks, integration total now 170). **All 31 smokes green via `pnpm smoke:all`** (15 data-layer + 13 UI + 1 integration-vault + 1 deploy + the new bill-history smoke = 31 suites; ~1,494 checks across 31 suites). tsc clean. Dev server live on testnet default. Predecessor: `ed4129b` (handoff + COORDINATION reflect Cluster 7.4). Spec: `00-CLUSTER-7.5-PER-BILL-AUDIT-DRILLDOWN.md`.
>
