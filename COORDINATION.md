...
> Last update: 2026-08-30 05:35 CDT (Cluster 7.6 — **Real-time audit log updates (SSE) — visible UI, no schema change** — committed on top of `f980240` (session-scratch cleanup) + new cluster commit. The `/vault/audit` and `/vault/bills/[id]/history` pages now surface new audit log rows in real time via Server-Sent Events. New `GET /api/vault/audit/stream` route returns `text/event-stream`; `recordVaultAudit` (in `src/lib/vault/db.ts`) publishes every new row to a process-local `EventEmitter` pinned to `globalThis.__COMPASS_AUDIT_BUS__`; the route subscribes + filters by `userId` (and optionally `?billId=` for the history page) and forwards every new event to its open connections. New `useAuditStream` client hook (`src/lib/vault/use-audit-stream.ts`) opens an `EventSource`, auto-reconnects on error, parses SSE messages, and applies the page's `ignoreActionTypes` for the self-feedback guard. New `LiveAuditTable` + `LiveBillEventTable` client components wrap the shared `AuditTableView` + `BillEventTableView` (extracted in this cluster so the server and live tables share one source of truth for the row JSX). Both wrappers preserve the server-rendered initial rows, prepend streamed rows that pass the page's filter, and cap the list at `take`. The page's own write-after-read meta events (`vault.audit_log_viewed`, `vault.bill_history_viewed`) are ignored by the hooks so the same visit's table doesn't visibly loop on them. New dev-only `POST /api/dev/audit-log-write` endpoint (gated by `NODE_ENV !== "production"`, `actionType` must start with `smoke.*`) calls `recordVaultAudit` so the smoke can fire the bus from outside the dev server's process — the bus is per-process, so a direct `prisma.auditLog.create()` from a separate Node process would not cross the boundary. Extracted `src/lib/vault/audit-log-shared.ts` (pure types + helpers — `colorForActionType`, `billHistoryHrefForAuditRow`, `parseAuditLogFilter`, `AuditLogFilter`, `AuditLogRow`, `BillHistoryFilter`); `audit-log.ts` re-exports for back-compat with existing server-side imports. New smoke `tests/smoke-sse-audit-log.mjs` (30 checks — self-contained via the dev endpoint, covers auth gate, hello message, end-to-end write→bus→SSE→client, `?billId=` filter, heartbeat, cleanup, meta-event guard, source-file checks). `package.json` smoke script includes `smoke-sse-audit-log.mjs`. New "Phase 4.0 M6 — Real-time audit log updates (SSE)" section in `tests/integration-vault.mjs` (13 new checks, integration total now 183). All previous smokes updated to accept the new `vault-audit-table-live` + `vault-bill-history-table-live` data-testids (legacy `vault-audit-table` + `vault-bill-history-table` still accepted for back-compat). **All 31 smokes green via `pnpm smoke:all`** (~1,522 checks across 31 suites). tsc clean. Dev server live on 127.0.0.1:3000 (testnet default; flip `VAULT_CHAIN_ID=8453` in `.env.local` to target mainnet). Spec: `00-CLUSTER-7.6-SSE-AUDIT-LOG.md`. **Upgrade path**: a multi-server / multi-region deploy would swap the in-process bus for a Postgres `LISTEN`/`NOTIFY` trigger; the SSE route handler + client hook stay the same. Single-server assumption is the documented contract. **Next cluster**: see "Next cluster" in `HANDOVER.md` — recommended: `?from=YYYY-MM-DD&to=YYYY-MM-DD` date range filter on `/vault/audit` (visible-UI, small surface, no infra).
>
