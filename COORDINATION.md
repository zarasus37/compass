...
- **Handed to**: next session — start by reading this file + `00-DESIGN.md`. The recommended next cluster is **5.0 Part B — LLM provider persistence**: the LLM engine from Part A is in-memory keyed by userId (src/lib/onboarding/state.ts); the next cluster replaces the in-memory store with a Prisma `FinancialIdentity` model + `OnboardingConversation` table, and adds an L1 rules fallback so the system still works when the LLM is offline or out of credit. After 5.0 Part B, the queue is: 5.1 chat UX (OnboardingGate + `/onboarding` page + ChatSurface + ProgressRail); 5.2 wire dashboard to identity (every surface reads from materialized records; seed moves to "demo mode"); 5.3 ongoing advisor via Ollama (post-onboarding "ask me anything" surface, same `callAgent` interface, privacy-first). Mavis is live (verified on 2026-08-25 with real CFP-grade responses; credits added). `LLM_PROVIDER` env switch works: `mock` for deterministic smokes, `mavis` for production, `ollama` for the local advisor. `.env.local.example` is committed; the actual `.env.local` is gitignored. The design system is **Sovereign Monad / vessel (v6)**, superseding Component Oracle Terminal. `tsc --noEmit` is clean; 15 smokes green at 513/513 (smoke-period 46, smoke-onboarding-agent 42, smoke-sidebar 60, smoke-topbar 102, smoke-bottom-dock 70, smoke-goals 36, smoke-deprecated 42, smoke-glossary 32, smoke-rebalance 5, smoke-reset-seed 8, smoke-engine-toggle 7, smoke-vessel-feed 7, smoke-horizon-strip 14, smoke-alert-bay 22, smoke-visual-finish 20); `smoke-auth.mjs` pre-existing failing — unrelated; dev server on 127.0.0.1:3000.
