...
> Last update: 2026-08-28 (Cluster Vault 4.0 M4 ✅ DONE — off-ramp gateway shipped; commit + full breakdown in `_commit_44.txt`). Adds `OffRampGateway` orchestrator (`src/lib/vault/gateway.ts`) with the 7-condition `canExecute` gate (status, vault pause, execution window, amount cap, settlement reserve, approver, pending attempt) + adapter chain resolution (preference → Spritz → Monto → Manual Push as the non-negotiable safety path). 3 new server actions in `src/lib/vault/server.ts`: `executeBillPaymentAction` (the "Execute now" path — drives the full EARNING → BEGIN_SETTLEMENT → PREPARING_SETTLEMENT → EXECUTE → EXECUTING → CONFIRM_SETTLED/MANUAL_ACTION_REQUIRED/FAIL_FINAL flow), `retryBillPaymentAction` (for degraded states), `confirmManualPaymentAction` (for out-of-band manual settlements; transitions to SETTLED with providerName "manual" and the user's reference). 2 new client components: `ExecuteBillButton` (with server-computed disabledReason tooltip) and `BillRecoveryPanel` (recovery UI for the 5 degraded states with [RETRY] + [MARK MANUALLY PAID]). New dev test endpoint at `/api/vault/execute-bill`. 2 new audit `actionType` values: `vault.payment_executed`, `vault.payment_manually_confirmed`. integration-vault 163/163 (was 144, +19 for M4), smoke-vault 77/77 (was 74, +3 for M4), all 25 adjacent smokes green, tsc clean. **M4 design decisions** (per user sign-off): (1) stubbed adapters stay (Q1 — real provider API is a separate later cluster; gateway is provider-agnostic), (2) manual "Execute now" only (Q2 — cron / scheduled trigger is a follow-on cluster). The 5-min in-flight check from the spec was dropped — the unique constraint on (providerName, idempotencyKey) plus the per-minute key rotation handle correctness, and a 5-min backoff blocked legitimate retries. Predecessors: Vault 4.0 M3 (real Aave V3 yield). Vault 4.0 M2 (USDC funding). Vault 4.0 M1 (Safe deploy). dev server on 127.0.0.1:3000.
