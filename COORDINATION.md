...
> Last update: 2026-08-29 05:00 CDT (Cluster 7.0 — **Vault preferences hub (visible UI)** — commit `38eb7e8`. Closes the "Vault prefs UX is light" gap from the HANDOVER. New `/vault/preferences` page consolidates the user's vault policy in one place: 4-cell policy summary (yield routing, risk disclosure, auto bill-pay, vault status) + yield routing picker + risk disclosure section (with new re-acknowledge flow) + schedule summary card. Three new components: `PolicySummaryCard`, `ScheduleSummaryCard`, `RevokeRiskAckButton`. New server action `revokeRiskDisclosureAction` sets `riskAcknowledgedAt = null` + writes a `vault.risk_unacknowledged` audit row. Re-skinned `RiskDisclosure` unack state + `SchedulerIndicator` + schedule page chrome to vessel tokens (additive map). Sidebar adds the `Preferences` entry under `// Ledger`; the `Vault` item now has `match: "exact"` so it doesn't light up on `/vault/preferences` or `/vault/schedule`. New `tests/smoke-vault-prefs.mjs` (65 checks) covers the page, the round-trip, the sidebar, the source-file re-skin, and the audit log row. **All 27 smokes green via `pnpm smoke:all`** (12 data-layer + 13 UI + 1 integration-vault + 1 deploy = 27; ~1,355 checks; smoke-vault-prefs is the new one). tsc clean. Dev server live. **Visible-UI half of the cluster; no engine changes, no new DB models, no new dependencies.** Predecessor: Cluster 6.0 fix-up commit `8ef4f8f`. Spec: `00-CLUSTER-7.0-VAULT-PREFS-UX.md`. Out of scope (deferred): rebalance cadence (spec doesn't define it), per-bill auto re-ack trigger, strategy-description editing, vault-pause-from-prefs. The remaining `var(--terminal-cyan)` / `var(--surface)` / `var(--line)` / `var(--line-soft)` / `var(--warn)` references in the un-migrated shells of `/vault/page.tsx` are tracked for the broader visual finish pass.)
>
> (Previous: 2026-08-29 03:50 CDT — Cluster 6.0 fix-up — **VaultSchedule in Prisma schema** — commit `8ef4f8f`. Same OffRampGateway + Aave V3 supply/withdraw flow as Cluster 6.0/M1-M4, but pointed at Base mainnet. Replaced the hardcoded `baseSepolia.id` check in `src/lib/vault/safe-deploy.ts` with a `CHAIN_TABLE` indexed by chainId; mirrored as `AAVE_CHAIN_TABLE` in `aave.ts`. Canonical mainnet addresses cross-referenced against the Aave address book + safe-deployments: Aave V3 Pool `0xa238dd80c259a72e81d7e4664a9801593f98d1c5`, Circle USDC `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913`, Safe singleton v1.3.0 `0x69f4D1788e39c87893C980c06EdF4b7f686e2938`. aUSDC stays dynamic via `Pool.getReserveData`. New read-only `GET /api/vault/chain-config` endpoint returns the resolved config; whitelisted in middleware. `/vault` StatusStrip now surfaces a gold `MAINNET` chip on chainId 8453 (priority over green LIVE). `prod.ts` refuses to start in production with `VAULT_CHAIN_ID=84532` (testnet). `.env.production.example` documents both modes — mainnet block is the live default, testnet block is a comment. **All 26 smokes green via `pnpm smoke:all`** (1 auth + 10 data-layer + 13 UI + 1 integration-vault + 1 deploy = 26; ~1,300 checks; smoke-deploy went from 72 to 95 checks). tsc clean. Dev server live on 127.0.0.1:3000 (currently on testnet default; flip `VAULT_CHAIN_ID=8453` in `.env.local` to target mainnet). Predecessor: Cluster 6.0 (commit `62b8528`). Spec: `00-CLUSTER-6.0.1-MAINNET-VAULT.md`. **Real mainnet deploy is still a manual gate** — the deployer EOA needs real ETH on Base, the [DEPLOY] button walks the same path the testnet does, but a wrong address or a misconfigured signer loses principal. No real mainnet deploy was performed in this cluster.
>
