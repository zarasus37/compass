...
> Last update: 2026-08-31 04:55 CDT (Cluster 7.11 — live activity ticker in the sidebar). New `LiveActivityTicker` client component subscribes to the existing C7.6 SSE bus + `useAuditStream` hook (no new infra); renders the last 3 vault events as a live feed under the `// Ledger` chapter in the sidebar on every (app) page. New shared `LiveTickerEvent` type + exhaustive `humanizeVaultAction` map covering all 30 vault action types + `formatRelativeTime` helper in `audit-log-shared.ts`. Meta events dropped on both seed and stream (`LIVE_TICKER_IGNORED_TYPES` set is the source of truth). New `src/lib/vault/audit-action-types.ts` extracts the `VaultAuditActionType` union + `VAULT_AUDIT_ACTION_TYPES` runtime array out of `db.ts` so the client can import the union without `server-only`. `AppSidebar` accepts a new optional `tickerInitialRows?: AuditLogRow[]` prop; `NavChapter` gets an optional `slot?: ReactNode` field; the `// Ledger` chapter renders `<LiveActivityTicker />` between the chapter label and the first nav item (silent when 0 events). Both `(app)/layout.tsx` and `src/app/page.tsx` (dashboard) read `getAuditLog(user.id, { take: 3 })` and pass to `AppSidebar`. New rows flash `+1 NEW` for 2s; billId rows deep-link to `/vault/bills/<id>/history`; `aria-label="Recent vault activity"` + `aria-live="polite"`. New `tests/smoke-live-ticker.mjs` (38 checks) covers page surface + bus round-trip + source-file checks. New `tests/integration-vault.mjs` Phase 4.0 M11 (~35 checks) covers the wiring. All 20 data-layer smokes (~1,005) + integration-vault 299 + smoke-deploy 102 = ~1,800 checks, ALL GREEN. tsc clean. Visible-UI payoff: every signed-in page shows a live feed of the last 3 vault events under `// Ledger` — C7.6 SSE infra finally has a user-visible surface in the most-trafficked chrome. — **Cron alert surface (M10) — minimal visible-UI + ops observability** — committed on top of `bec5d5c` (Cluster 7.9) + new cluster commit. When the audit log retention cron (M8) returns `usersFailed > 0`, each failed user now produces a `vault.cron_prune_failure` audit row (durable record, visible in the audit page table) + an optional webhook POST to `CRON_ALERT_WEBHOOK_URL` (Sentry envelope / PagerDuty Events API v2 / generic JSON — auto-detected from the URL). The audit row is the source of truth; the webhook is a real-time shortcut. New `src/lib/vault/audit-log-alerts.ts` exports `recordCronAlert({ userId, kind, error, context? })` + `getRecentCronAlerts(userId, limit?)`. The bulk helper in `audit-log-cron.ts` calls `recordCronAlert` for each ERROR result via `Promise.allSettled` (parallel, failure-isolated). New dev-only `POST/GET /api/dev/cron-alerts` endpoint for the smoke + manual ops checks (same `NODE_ENV` gate as the other dev endpoints). New `vault.cron_prune_failure` action type in the `recordVaultAudit` union (the audit page renders it like any other event — `colorForActionType` picks the color automatically). `.env.production.example` documents the optional `CRON_ALERT_WEBHOOK_URL` + `CRON_ALERT_PAGERDUTY_ROUTING_KEY` env vars. New `tests/smoke-cron-alerts.mjs` (36 checks) covers the dev endpoint round-trip + source-file checks. New `tests/integration-vault.mjs` Phase 4.0 M10 (22 checks) covers the same wiring at the integration level + wire checks. All 20 data-layer smokes (951 checks) + integration-vault 261 + smoke-deploy 102 = ~1,730 checks, ALL GREEN. tsc clean. **Webhook format auto-detection**: Sentry (`url contains sentry.io`) → Sentry envelope with `event_id`, `level`, `message`, `extra`; PagerDuty (`url contains pagerduty.com` or ends in `/v2/enqueue`) → Events API v2 with `routing_key`, `event_action`, `dedup_key: compass-cron-prune_failure-<userId>`, `payload.summary`; anything else → generic JSON `{ event, severity, message, details }`. **Failure isolation**: each alert runs in `Promise.allSettled` so one failed webhook (Sentry outage) doesn't break the others; each webhook has a 2s hard timeout (Vercel's 10s cron budget is plenty of headroom); webhook failures are logged to stderr (URL masked — never log the Sentry key or PagerDuty routing key in plaintext) but don't throw. The audit row is the durable record. **Production setup**: one env var. Set `CRON_ALERT_WEBHOOK_URL` to a Sentry DSN or PagerDuty Events API URL. For PagerDuty, also set `CRON_ALERT_PAGERDUTY_ROUTING_KEY` to the integration key. When unset, only the audit row is written (no webhook) — the alert surface is a no-op, not an error. **No end-to-end webhook test in the smoke** — a real webhook test would require a mock HTTP server in the dev process OR a real Sentry test project. The smoke verifies the audit-row path (which IS the durable record) + source-file checks for the webhook code path. **No vault cron alerts yet** — only the audit log retention cron goes through this surface. The vault auto bill-pay scheduler (C6.0) errors are still in the per-user `result.error` field but not alerted. The `recordCronAlert` API is open — adding `kind: "vault_billpay_failure"` is a future cluster. Predecessor: Cluster 7.9 (commit `bec5d5c`). Spec: `00-CLUSTER-7.10-CRON-ALERTS.md`. **The remaining candidates from the user list**: (2) CRON_SECRET enforcement on Vercel — Vercel function middleware that verifies the `vercel-cron/1.0` user-agent; (3) per-user retention override — premium tier with longer retention horizon. Or pick a different cluster from the candidate list in HANDOVER.)
>
>
