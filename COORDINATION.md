...
> Last update: 2026-09-06 (Cluster 7.16 — Mom-ready / v1 launch). Adds the launch shape: PWA manifest + 4 icons + service worker (so mom can install the app on her phone from the browser), `scripts/seed-admin.mjs` (idempotent CLI that creates her account from `ADMIN_EMAIL` + `ADMIN_NAME` + `ADMIN_PASSWORD` env vars, run as part of the Vercel build), `scripts/reset-password.mjs` (CLI for xKryptic to reset mom's password from his terminal when she forgets; invalidates all sessions on reset), `src/components/pwa/ServiceWorkerRegistrar.tsx` (dev-skipped) + `src/components/pwa/InstallPrompt.tsx` (iOS Safari tip + Android `beforeinstallprompt`), `src/app/layout.tsx` wired with manifest + theme color + apple-touch-icon, and `.env.production.example` documents the new `ADMIN_EMAIL` / `ADMIN_NAME` / `ADMIN_PASSWORD` block. **Vault defers to v1.1** (Safe deployer wallet + Base mainnet ETH is a real product surface but not what "single user, budgeting app" means). Runbook at `00-MOM-LAUNCH-RUNBOOK.md` walks xKryptic through the external-account work (GitHub repo push, Neon DB, Vercel project + env vars, deploy, send mom the URL). **Workflow**: `main` = mom's deployed app, feature branches = xKryptic's cluster work (each gets a Vercel preview URL), merge `feature → main` = ship to mom. **smoke:deploy extended to 137/137 green** (§15 cluster 7.16: PWA + icons + SW + registrar + install prompt + seed-admin + reset-password + ADMIN_* env contract). Cluster 7.15 spec (per-bill payment history sparkline) is paused on disk; resume when mom is live.
>
