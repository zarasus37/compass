...
> Last update: 2026-08-31 00:15 CDT (Cluster 7.8.2 — **Vercel cron schedule (M9) — pure infra, no visible UI** — committed on top of `b7ef8cf` (Cluster 7.8.1) + new cluster commit. A `vercel.json` at the project root wires the production schedule for both crons. `/api/cron/audit-log-prune` at `0 3 * * *` (03:00 UTC daily — the audit log retention); `/api/cron/vault` at `*/5 * * * *` (every 5 minutes — the vault auto bill-pay scheduler). **Vercel sends GET, not POST** to the paths in `vercel.json:crons`; the vault endpoint's pre-M9 `GET` was a debug `dueCount` handler that did NOT fire the scheduler — useless for Vercel. The new vault `GET` delegates to `POST` (same work: find due users, run the scheduler, return the summary). The audit log endpoint already had `GET → POST` delegation in C7.8.1. The endpoints run without `CRON_SECRET` in Vercel — the cron user-agent (`vercel-cron/1.0`) is the implicit auth (a future cluster can add a Vercel function middleware to verify it). **No "exactly once" guarantee**: two cron invocations at the same minute would both fire the scheduler; the functions are idempotent — the second one returns `usersProcessed: 0` / `totalRolledUp: 0`. No double-delete, no data loss. **No `vercel.json` for non-cron config** (buildCommand, framework, env-var overrides) — Vercel detects Next.js automatically; the rest is the default. **Why both crons in M9?** The audit log retention (C7.8/M8) and the vault auto bill-pay (C6.0) were both designed for Vercel cron but neither had a `vercel.json` to point at them — M9 wires both in one place. New `tests/integration-vault.mjs` Phase 4.0 M9 (13 checks) verifies the `vercel.json` shape (exists + valid JSON + has `crons` array + has both expected paths + schedules are well-formed) + the vault GET delegation + wire checks (GET /api/cron/vault returns 200 with the new shape; GET /api/cron/audit-log-prune returns 200). New `tests/smoke-deploy.mjs` §14 (7 checks) verifies the same `vercel.json` wiring at the deploy-readiness level. All 19 data-layer smokes + 13 UI smokes + integration-vault 239 + smoke-deploy 102 = ~1,680 checks, ALL GREEN (1 pre-existing audit-log date-TZ miss, down from 3 earlier — environmental TZ math; not from M9). tsc clean. Dev server live on testnet default. Predecessor: Cluster 7.8.1 (commit `b7ef8cf`). Spec: `00-CLUSTER-7.8.2-VERCEL-CRON-SCHEDULE.md`. **The first production deploy after M9 will start running the audit log retention nightly AND the vault auto bill-pay every 5 minutes** — both behaviors were wired in earlier clusters (C6.0 and C7.8), M9 just turns them on for production. The pre-existing 7.7 audit-log TZ miss is the only remaining miss; a 7.9 cluster is the natural follow-on to fix the smoke's UTC-vs-local comparison.)
>
>
> (Previous: 2026-08-30 22:25 CDT (Cluster 7.8.1 — **Audit log retention cron (M8) — pure infra, no visible UI** — committed on top of `8f7b23b` (Cluster 7.8) + new cluster commit. A nightly job that calls `pruneAuditLog` for every user, with the same pattern as the existing vault cron. New `pruneAuditLogForAllUsers(opts?)` bulk helper in `src/lib/vault/audit-log-cron.ts` iterates every user and calls `pruneAuditLog(userId, ...)` for each; each user's prune is wrapped in try/catch so one user's failure doesn't abort the batch — the `usersFailed` count + per-user `error` field let the operator see exactly which users failed. Returns `{ ok, usersProcessed, usersFailed, totalRolledUp, totalDeleted, totalRollupRows, results, retentionDays, now }` (same shape pattern as the vault cron's response). New `POST /api/cron/audit-log-prune` endpoint (POST + GET delegate to the bulk helper; auth: same `CRON_SECRET` bearer pattern as `/api/cron/vault` — required in prod, skipped in dev). New `scripts/cron-audit-prune-dev.mjs` long-running poller, configurable via `AUDIT_LOG_PRUNE_POLL_MS` env (default 24h; override to 5-30s in dev for fast feedback); logs `no users due` when nothing to prune, `processed=N rolledUp=X deleted=Y` when work happened, `ERROR <userId>...` per failed user, plus a final shutdown line on Ctrl-C. New `cron:dev:audit` npm script. The middleware gained `/api/cron` in the public prefixes list — **a pre-existing bug where the dev scheduler was being silently redirected to `/login` is now fixed** (the script's try/catch + JSON parse error tolerance hid the bug for months; every dev poll silently failed). `.env.production.example` documents the optional `CRON_SECRET` env var. No visible-UI payoff (pure infra); the user sees nothing new, but the live `AuditLog` table is now bounded at ~90 days per user. **Idempotency + concurrency-safety**: two cron processes firing simultaneously both call `pruneAuditLogForAllUsers`; the second one finds nothing to prune (everything is already in the rollup) and returns `totalRolledUp: 0` — no double-delete, no data loss. New `tests/smoke-cron-audit-log-prune.mjs` (30 checks) covers: POST + GET return 200 with the right shape; the smoke user appears in `results` with `status: "PRUNED"` + `rolledUp >= 2`; 3 in-window sentinels remain in the live table; 1 rollup row for `(100d, smoke.cron_prune.old)` with `count=2`; idempotent 2nd call returns `status: "NOOP"`; source-file checks for the bulk helper, route, dev script, npm script, and middleware fix. New `tests/integration-vault.mjs` Phase 4.0 M8 (23 checks) covers the same wiring at the integration level + POST/GET wire checks. All 19 data-layer smokes + 13 UI smokes + integration-vault 226 + smoke-deploy = ~1,620 checks, ALL GREEN. tsc clean. Predecessor: Cluster 7.8 (commit `8f7b23b`). Spec: `00-CLUSTER-7.8.1-AUDIT-LOG-CRON.md`. **The next cluster is `vercel.json` with the cron schedule** — the endpoint is callable from any cron system; the schedule (`0 3 * * *` recommended) is the only missing piece for production.)
