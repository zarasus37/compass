...
> Last update: 2026-08-31 05:25 CDT (Cluster 7.11.1 — live activity ticker polish: semantic tone colors + reconcile-on-reconnect). Three real wins from Polar's v2-amended spec, folded in as a polish commit on top of the 7.11 ticker that shipped earlier today. (1) **Semantic tone colors** — replaced the 7.11 djb2 hash (3-row confetti) with a 4-tone semantic map: `good`=green (`--ok`), `watch`=orange (`--vessel-watch`), `bad`=red (`--vessel-over`), `neutral`=dim (`--ink-3`). New `HumanizeTone` type + `TONE_FOR` `Record<VaultAuditActionType, HumanizeTone>` map + `TONE_COLOR` CSS-var map in `audit-log-shared.ts`. The audit page keeps djb2 (50+ row table is where type-distinct earns its keep). (2) **Hide unmapped types** — `humanizeVaultAction` return shape changed from `string` to `{ text, tone } | null`; the ticker silently drops `null` instead of rendering an "event happened" placeholder. (3) **Reconcile on reconnect** — `useAuditStream`'s docblock is explicit that reconnect does NOT replay missed events; the ticker now hits the new `GET /api/vault/audit/recent?take=N` endpoint on the `reconnecting`/`closed` → `live` transition to backfill any rows that arrived during the disconnect window. New `LiveTickerEvent.tone` field carries the semantic tone; the ticker renders `data-tone="good|watch|bad|neutral"` on every row for the smoke. `tests/smoke-live-ticker.mjs` grew from 38 to 64 checks (tone rendering + new endpoint round-trip + reconcile source-file checks + meta-event cross-chain cleanup); `tests/integration-vault.mjs` M11 grew from 35 to 53 checks. All 20 data-layer smokes (~1,070) + integration-vault 310 + smoke-deploy 102 = ~1,800 checks, ALL GREEN (the pre-existing `smoke-goals-db.mjs` seed-order flake is unrelated — reproduces on `a8639d6` too). tsc clean. Visible-UI payoff: the ticker dots now read as "what kind of event" instead of "which type" — red=needs me, green=good news, orange=in progress, dim=ambient — and a momentary SSE disconnect no longer drops events. No schema change, no new env vars, no breaking changes for non-ticker consumers. Predecessor: `a8639d6` (Cluster 7.11). Spec: no new spec file; this commit is the polish. — **Cluster 7.11 (earlier today, commit `a8639d6`)** — live activity ticker in the sidebar. New `LiveActivityTicker` client component subscribes to the existing C7.6 SSE bus + `useAuditStream` hook (no new infra); renders the last 3 vault events as a live feed under the `// Ledger` chapter in the sidebar on every (app) page. New shared `LiveTickerEvent` type + exhaustive `humanizeVaultAction` map covering all 30 vault action types + `formatRelativeTime` helper in `audit-log-shared.ts`. Meta events dropped on both seed and stream (`LIVE_TICKER_IGNORED_TYPES` set is the source of truth). New `src/lib/vault/audit-action-types.ts` extracts the `VaultAuditActionType` union + `VAULT_AUDIT_ACTION_TYPES` runtime array out of `db.ts` so the client can import the union without `server-only`. `AppSidebar` accepts a new optional `tickerInitialRows?: AuditLogRow[]` prop; `NavChapter` gets an optional `slot?: ReactNode` field; the `// Ledger` chapter renders `<LiveActivityTicker />` between the chapter label and the first nav item (silent when 0 events). Both `(app)/layout.tsx` and `src/app/page.tsx` (dashboard) read `getAuditLog(user.id, { take: 3 })` and pass to `AppSidebar`. New rows flash `+1 NEW` for 2s; billId rows deep-link to `/vault/bills/<id>/history`; `aria-label="Recent vault activity"` + `aria-live="polite"`. New `tests/smoke-live-ticker.mjs` (38 checks) covers page surface + bus round-trip + source-file checks. New `tests/integration-vault.mjs` Phase 4.0 M11 (~35 checks) covers the wiring. All 20 data-layer smokes (~1,005) + integration-vault 299 + smoke-deploy 102 = ~1,800 checks, ALL GREEN. tsc clean. Visible-UI payoff: every signed-in page shows a live feed of the last 3 vault events under `// Ledger` — C7.6 SSE infra finally has a user-visible surface in the most-trafficked chrome.
>
>
