...
> Last update: 2026-08-25 (post-Cluster-5.2 — Wire dashboard to identity: new Identity Summary card on the dashboard that reads from FinancialIdentity + child tables (primary income, debt total, top goals, risk profile, audit); 3 visual states (empty/partial/full); 1-3 clickable growth-oriented suggestions (Grow Goal, Crush Debt, Review Plan) that route to the relevant deep pages (v1 of the headline-numbers-need-growth-suggestions pattern); the card is wired into the dashboard catalog as defaultOn with a status pill ([OK] INDEXED / [WARN] PARTIAL / [ ] NOT SET); new "Use demo data" button on /onboarding (above the chat input on first visit) that posts to /api/onboarding/seed-demo which creates a completed FinancialIdentity with the canonical seed data + redirects 303 to /; the rest of the dashboard widgets keep the in-memory seed (a follow-up cluster will project FinancialIdentity → production tables for the envelopes/bills/allocation-plan shapes that don't match 1:1); total 15 smokes at 551/551)
...
- **Cluster 5.2.5 — Projection step + remaining surfaces**: project the FinancialIdentity into the production tables (Account, Envelope, Bill, AllocationPlan, AllocationRule) so the other dashboard widgets can read from Prisma too. The Identity Summary card already reads from FinancialIdentity; the rest still use the in-memory seed. After projection, the period / envelopes / bills / allocation / insights / period pages can switch from `liveEnvelopes()` etc. to the production tables. This is a one-time projection that runs when the chat completes (or via the demo-mode endpoint).
