...
> Last update: 2026-08-25 (post-Cluster-4.2 — /goals goal-type filtering: GoalType enum (EMERGENCY/INVEST) added, new Invest goal seeded, /goals?kind=emergency|invest deep-links wired, kind tabs + per-card kind badges, new smoke at 36 checks; total 11 smokes at 363/363)
...
- **Cluster 4.3 — Old page deletion.** The dead `/recurring`, `/subscriptions`, `/investments`, `/emergency`, `/invest`, `/settings/habit-quiz` files are now in `src/app/(app)/_deprecated/` (committed in Cluster 4.0). They can be hard-deleted once a Trash tool is available; the redirects in `next.config.ts` make them inert either way.
