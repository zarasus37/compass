...
> Last update: 2026-08-29 05:45 CDT (Cluster 7.1 — **⌘K command palette (visible UI)** — commit `d71c224`. Closes the COORDINATION "Handoff menu" entry for a global search/jump drawer. ⌘K (or Ctrl+K on Windows/Linux) opens a centered palette from anywhere on a signed-in page. Type to filter across 21 static routes + the ~30 envelopes/goals/debts/bills/accounts in the DB; arrow keys navigate, Enter jumps, Escape closes. New files: `CommandPalette.tsx` (the modal drawer), `CommandPaletteProvider.tsx` (the open/close state + global keyboard), `SearchButton.tsx` (the discoverability affordance in the top bar), `routes.ts` (21 static routes with terminal-voice sub-text), `match.ts` (pure case-insensitive substring + word-boundary-boost matcher with exact > starts-with > word > substring ranking), `search-index.ts` (server function: 5 Prisma lists + the in-memory debt store mapped to PaletteItem), `types.ts` (the shared contract). New `GET /api/command-palette?q=…` endpoint returns the index + the server-side filtered results (used by the smoke; the palette reads the index as a server-component prop). The dashboard (`/`) gets its own provider since it lives outside `(app)/`. The TopAppBar's right column adds the `[⌘K] Search` button. New `tests/smoke-command-palette.mjs` (64 checks) verifies the button is on every signed-in page, the API shape, the static + dynamic item counts, the match ranking, and the auth gate. **All 28 smokes green via `pnpm smoke:all`** (13 data-layer + 13 UI + 1 integration-vault + 1 deploy = 28; ~1,419 checks; smoke-command-palette is the new one). tsc clean. **Visible-UI; no engine changes, no DB schema, no new dependencies.** Predecessor: Cluster 7.0 commit `38eb7e8`. Spec: `00-CLUSTER-7.1-COMMAND-PALETTE.md`. Out of scope (deferred): command execution ("create envelope" type actions — the hook has a placeholder .run() for future use), recent items ("last 5 visited"), fuzzy matching, full onboarding tour.)
>
> (Previous: 2026-08-29 05:00 CDT — Cluster 7.0 — **Vault preferences hub (visible UI)** — commit `38eb7e8`. Same OffRampGateway + Aave V3 supply/withdraw flow as Cluster 6.0/M1-M4, but pointed at Base mainnet. Replaced the hardcoded `baseSepolia.id` check in `src/lib/vault/safe-deploy.ts` with a `CHAIN_TABLE` indexed by chainId; mirrored as `AAVE_CHAIN_TABLE` in `aave.ts`. Canonical mainnet addresses cross-referenced against the Aave address book + safe-deployments: Aave V3 Pool `0xa238dd80c259a72e81d7e4664a9801593f98d1c5`, Circle USDC `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913`, Safe singleton v1.3.0 `0x69f4D1788e39c87893C980c06EdF4b7f686e2938`. aUSDC stays dynamic via `Pool.getReserveData`. New read-only `GET /api/vault/chain-config` endpoint returns the resolved config; whitelisted in middleware. `/vault` StatusStrip now surfaces a gold `MAINNET` chip on chainId 8453 (priority over green LIVE). `prod.ts` refuses to start in production with `VAULT_CHAIN_ID=84532` (testnet). `.env.production.example` documents both modes — mainnet block is the live default, testnet block is a comment. **All 26 smokes green via `pnpm smoke:all`** (1 auth + 10 data-layer + 13 UI + 1 integration-vault + 1 deploy = 26; ~1,300 checks; smoke-deploy went from 72 to 95 checks). tsc clean. Dev server live on 127.0.0.1:3000 (currently on testnet default; flip `VAULT_CHAIN_ID=8453` in `.env.local` to target mainnet). Predecessor: Cluster 6.0 (commit `62b8528`). Spec: `00-CLUSTER-6.0.1-MAINNET-VAULT.md`. **Real mainnet deploy is still a manual gate** — the deployer EOA needs real ETH on Base, the [DEPLOY] button walks the same path the testnet does, but a wrong address or a misconfigured signer loses principal. No real mainnet deploy was performed in this cluster.
>
