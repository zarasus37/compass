...
> Last update: 2026-08-26 (Cluster Vault 4.0 M3 ✅ DONE — real yield via Aave V3 on Base Sepolia, see HANDOFF — Vault 4.0 M3 below for commit hash + full surface). M3 replaces the `aave` yield adapter's hardcoded 4.25% with a real on-chain read of Aave V3's `Pool.getReserveData(USDC).currentLiquidityRate` (ray → APY via `(rateRay * SECONDS_PER_YEAR) / RAY / 1e18`, clamped to [0,1] for safety; `VAULT_AAVE_APY` env var is the silent fallback if the RPC is unreachable). The Safe deposits USDC into Aave V3 via two new Safe transactions: `approve(usdc, pool, type(uint256).max)` on first supply (subsequent deposits skip the approval via a `getCurrentAllowance` pre-flight) and `pool.supply(usdc, amount, safe, 0)` which mints aUSDC to the Safe. The `[DEPOSIT] $X → AAVE` button on /vault calls `depositSafeUsdcAction`; the `[WITHDRAW] $X aUSDC` button calls `withdrawSafeUsdcAction` which burns aUSDC + sends USDC back to the Safe. The vault principal KPI cell on the status strip now shows an additional "· aUSDC $X.XX earning" sub line when the aUSDC balance > 0. Both buttons are hidden in MOCK state. Two new audit action types: `vault.aave_supply`, `vault.aave_withdraw`. Three new schema columns on `VaultAccount`: `onChainAUsdcBalanceCents Int @default(0)`, `aUsdcBalanceRefreshedAt DateTime?`, `aUsdcTokenAddress String?` (resolved dynamically from `Pool.getReserveData(USDC).aTokenAddress` on first supply + cached on the row). The Aave lib lives in `src/lib/vault/aave.ts` (~700 lines: `getAaveChainConfig`, `getReserveApy` (on-chain read), `getAUsdcTokenAddress`, `getAaveUsdcBalance`, `getAUsdcBalance`, `getCurrentAllowance`, `buildSupplyCalldata`, `buildWithdrawCalldata`, `buildApproveCalldata`, `executeSafeTransaction` (Protocol Kit wrapper), `supplySafeUsdc`, `withdrawSafeUsdc`). 5 new db accessors: `setAUsdcBalance`, `recordAaveSupply`, `recordAaveWithdraw`, `findAaveSupplyByIdempotencyKey`, `findAaveWithdrawByIdempotencyKey`. 3 new server actions: `depositSafeUsdcAction`, `withdrawSafeUsdcAction`, `refreshAUsdcBalanceAction`. 2 new client components: `DepositButton` + `WithdrawButton` (preset chips $50/$100/$500/$1,000 + custom input, per-click nonce, `// aave-usdc: ready|needs faucet` status line). 2 new env vars: `VAULT_AAVE_POOL_ADDRESS` (default `0x8bAB6d1b75f19e9eD9fCe8b9BD338844fF79aE27` — Aave V3 Pool on Base Sepolia), `VAULT_AAVE_USDC_ADDRESS` (default `0xba50Cd2A20f6DA35D788639E581bca8d0B5d4D5f` — Aave's faucet USDC, distinct from M2's Circle USDC), `VAULT_AAVE_AUSDC_ADDRESS` (fallback aToken address), `VAULT_DEPOSIT_MAX_CENTS` (default $10k per call). integration-vault 144/144 (was 116, +28 for M3), smoke-vault 74/74 (was 71, +3 for M3), adjacent smokes green, tsc clean. **Remaining: M4 = off-ramp gateway (deferred to a later cluster).** Predecessors: Vault 4.0 M2 (`b7ea8d8` + `f23cd08`). Vault 4.0 M1 (`da71ae5`). Vault 3.5 (`7ea2a60`). dev server on 127.0.0.1:3000.
